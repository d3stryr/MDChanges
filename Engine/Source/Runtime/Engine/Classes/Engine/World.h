...
public:
	...
	TFunction<void(AActor*)> CustomPreSpawnInitialization;
};
...
{
public:
	using Super = FObjectSubsystemCollection<UWorldSubsystem>;

	FWorldSubsystemCollection();
	virtual ~FWorldSubsystemCollection();
private:
	virtual void OnSubsystemAdded(TNonNullPtr<USubsystem> Subsystem) override;
...
public:
	...
	/** Used to allow changing of visiblity transaction information in situations where overrides are applied after the world is created */
	UE_EXPERIMENTAL(5.8, "Experimental, API may change in the future")
	UE_API void InvalidateVisiblityTransactionRequestCachedVariables();
...
private:
	...
	/** Returns whether AddToWorld should be skipped on a given level */
...
private:
	/** Whether BeginPlay has been called on actors. Use the public accessors GetBegunPlay() & SetBegunPlay() */
	uint8 bBegunPlay:1;
...
private:
	...
	/** Whether FLumenSceneData::bTrackAllPrimitives can be set even if the world is a preview world. Used to make asset editor preview viewports work with Lumen. */
	uint8 bAllowLumenPrimitiveTrackingInPreviewWorld : 1;
...
public:
	...
	UE_DEPRECATED(5.8, "ChangeFeatureLevel is deprecated, use ShaderPlatformChanged instead")
	...
	UE_API void ShaderPlatformChanged(EShaderPlatform InShaderPlatform = EShaderPlatform::SP_NumPlatforms, bool bShowSlowProgressDialog = true);
...
public:
	...
	uint32 GetLaneId() { return LaneId; }
...
private:
	...
	DECLARE_TS_MULTICAST_DELEGATE(FOnSelectedLevelsChangedEvent);
	...
	/** a delegate that broadcasts a notification whenever the current feature level is changed */
...
public:
	...
	TSharedPtr<class FFXSystemInterface>		FXSystem;
	...
	// Storing PPVs safely is somewhat awkward due to historical API decisions.
	// We want to store IInterface_PostProcessVolume*, but callers do not always properly call the Remove function,
	// which leads to dangling pointers.
	// Most PPVs are UObject-based, and so can use TWeakInterfacePtr to detect object expiration.
	// The API signature historically allows for passing a raw pointer though, so we also support those cases for now.
	// TLDR: Variant type for PPV storage: UObject-based PPVs use TWeakInterfacePtr (detects stale entries),
	// non-UObject raw pointer PPVs are assumed always valid.
	using FPostProcessVolumeEntry = TVariant<
		TWeakInterfacePtr<IInterface_PostProcessVolume>,
		IInterface_PostProcessVolume*>;

	/** Iterator range over post process volumes in ascending priority order, filtering out invalid/expired entries. */
	struct FPostProcessVolumeIterator
	{
		struct FSentinel {};

		struct FIterator
		{
			FIterator(const TConstArrayView<FPostProcessVolumeEntry> InEntries, int32 InIndex)
				: Entries(InEntries), Index(InIndex)
			{
				AdvanceToValid();
			}

			IInterface_PostProcessVolume& operator*() const { return *Current; }
			...
			bool operator!=(FSentinel) const { return Current != nullptr; }

		private:
			UE_API void AdvanceToValid();

			const TConstArrayView<FPostProcessVolumeEntry> Entries;
			int32 Index;
			IInterface_PostProcessVolume* Current = nullptr;
		};

		explicit FPostProcessVolumeIterator(const TArray<FPostProcessVolumeEntry>& InEntries)
			: Entries(InEntries)
		{}

		FIterator begin() const { return FIterator(Entries, 0); }
		FSentinel end() const { return {}; }

	private:
		const TConstArrayView<FPostProcessVolumeEntry> Entries;
	};

	/** Inserts a UObject-based post process volume into the world in priority order */
	UE_API void AddPostProcessVolume(TWeakInterfacePtr<IInterface_PostProcessVolume> InVolume);

	UE_DEPRECATED(5.8, "Use AddPostProcessVolume(TWeakInterfacePtr<IInterface_PostProcessVolume>): for memory-safety, all IInterface_PostProcessVolume need to be UObject-based")
	 ...
	 *  Accepts both UObject and non-UObject based PPVs.
	 */
	...
	/** Returns an iterator over post process volumes in ascending priority order, filtering out invalid entries. */
	FPostProcessVolumeIterator GetPostProcessVolumeIterator() const
	{
		return FPostProcessVolumeIterator(PostProcessVolumeEntries);
	}

	UE_DEPRECATED(5.8, "Use GetPostProcessVolumeIterator() instead")
	TArray<IInterface_PostProcessVolume*> PostProcessVolumes;


private:
	TArray<FPostProcessVolumeEntry> PostProcessVolumeEntries;
	void InsertPostProcessVolumeEntry(FPostProcessVolumeEntry InVolume);
	void RemovePostProcessVolume(int32 Index); // do not make public, indices are unstable
	/** Caches the size of each volume at the time of insertion, for sort determinism in case it later changes.  One for one with PostProcessVolumeEntries above. */
...
public:
	...
	/**
	 *  Deproject a 2D screen-space coordinate using the supplied projection data, then line-trace from the resulting world-space ray.
	 *
	 *  @param  OutHit          First blocking hit found
	 *  @param  ProjectionData  View origin / rotation / projection matrix and view rect used for deprojection
	 *  @param  ScreenPosition  Screen-space coordinate (pixels, relative to the view rect)
	 *  @param  TraceChannel    The 'channel' the ray is in, used to determine which components to hit
	 *  @param  Params          Additional parameters used for the trace
	 *  @param  TraceDistance   Maximum distance to trace along the deprojected direction
	 *  @return TRUE if a blocking hit is found
	 */
	UE_API bool LineTraceFromScreenProjection(FHitResult& OutHit, const FSceneViewProjectionData& ProjectionData, const FVector2D& ScreenPosition, const ECollisionChannel TraceChannel, const FCollisionQueryParams& Params, const float TraceDistance) const;
...
public:
	...
	*/
	...
	/** Accessor for bAllowLumenPrimitiveTrackingInPreviewWorld. */
	bool AllowLumenPrimitiveTrackingInPreviewWorld() const
	{
		return bAllowLumenPrimitiveTrackingInPreviewWorld;
	}
...
public:
	...
	/** Returns current timeout used by AddToWorld */
	UE_API UE::FTimeout GetAddToWorldTimeout() const;

	/** Returns current timeout used by RemoveFromWorld */
	UE_API UE::FTimeout GetRemoveFromWorldTimeout() const;
...
public:
	...
	/* Snapshot material parameter collection retention state before garbage collection. */
	UE_API void OnPreGC();
...
public:
	...
	*/
	...
	/**
	 * Flush any pending parameter collection updates to the render thread.
	 */
...
public:
	...
	TOptional<UE::FTimeout> AddToWorldTimeout;
	TOptional<UE::FTimeout> RemoveFromWorldTimeout;
};
...
public:
	...
	DECLARE_TS_MULTICAST_DELEGATE_TwoParams(FWorldGetAssetTagsWithContext, const UWorld*, FAssetRegistryTagsContext);
	DECLARE_TS_MULTICAST_DELEGATE_TwoParams(FWorldGetAssetTags, const UWorld*, TArray<UObject::FAssetRegistryTag>&);
	...
	DECLARE_TS_MULTICAST_DELEGATE_TwoParams(FWorldCollectSaveReferencesEvent, UWorld*, FArchive&);
	static UE_API FWorldCollectSaveReferencesEvent OnCollectSaveReferences;

	static UE_API FWorldEvent OnPreRecreateScene;
	static UE_API FWorldEvent OnPostRecreateScene;
#endif // WITH_EDITOR
