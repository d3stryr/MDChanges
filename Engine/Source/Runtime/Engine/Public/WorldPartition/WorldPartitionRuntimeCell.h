...
class UWorldPartitionRuntimeCell : public UObject, public IWorldPartitionCell, public IWorldPartitionCookPackageObject
{
	...
	/** Thread-safe. Returns whether this cell's level is visible (added to world and rendering). */
	...
	/** Thread-safe resolve and cache of the effective wanted state for this cell's data layers. */
	...
	virtual FName GetCookPackageName() const PURE_VIRTUAL(UWorldPartitionRuntimeCell::GetCookPackageName, return NAME_None;);
	
	//~Begin IWorldPartitionCell Interface
	virtual TSet<FName> GetActorPackageNames() const override { return TSet<FName>(); }
	//~End IWorldPartitionCell Interface

	virtual void AddActorToCell(const FStreamingGenerationActorDescView& ActorDescView) PURE_VIRTUAL(UWorldPartitionRuntimeCell::AddActorToCell, );
	...
	virtual FWorldPartitionPackageHash GetGenerationHash() const override PURE_VIRTUAL(UWorldPartitionRuntimeCell::GetGenerationHash, return FWorldPartitionPackageHash(););
	...
	void SetIsCustomHLODPlaceholderCell(bool bInIsCustomHLODPlaceholderCell) { bIsCustomHLODPlaceholderCell = bInIsCustomHLODPlaceholderCell; }
	bool GetIsCustomHLODPlaceholderCell() const { return bIsCustomHLODPlaceholderCell; }
	
	const FGuid& GetGuid() const { return CellGuid; }
	void SetGuid(const FGuid& InCellGuid) { CellGuid = InCellGuid; }

	/** Diagnostic-only runtime-cell transition sidecar. Accessed on the game thread. */
	uint64 GetProvenanceCellTransitionId() const { return ProvenanceCellTransitionId; }
	EWorldPartitionRuntimeCellState GetProvenanceCellTransitionTarget() const { return static_cast<EWorldPartitionRuntimeCellState>(ProvenanceCellTransitionTarget); }
	void SetProvenanceCellTransition(uint64 InTransitionId, EWorldPartitionRuntimeCellState InTargetState) const
	{
		ProvenanceCellTransitionId = InTransitionId;
		ProvenanceCellTransitionTarget = static_cast<uint8>(InTargetState);
	}
	bool ClearProvenanceCellTransition(uint64 InExpectedTransitionId) const
	{
		if (ProvenanceCellTransitionId != InExpectedTransitionId)
		{
			return false;
		}
		ProvenanceCellTransitionId = 0;
		ProvenanceCellTransitionTarget = static_cast<uint8>(EWorldPartitionRuntimeCellState::Unloaded);
		return true;
	}
...
protected:
	inline const TArray<FName>& GetDataLayersInline() const { return DataLayers.ToArray(); }
	...
	/** Thread-safe read of the cached effective wanted state (without resolving). */
	EDataLayerRuntimeState GetCellEffectiveWantedStateRaw() const
	{
		if (!HasDataLayers()) { return EDataLayerRuntimeState::Activated; }
		UE::TReadScopeLock ReadLock(EffectiveWantedStateLock);
...
private:
	...
	UPROPERTY()
	bool bIsCustomHLODPlaceholderCell;
...
protected:
	...
	mutable FTransactionallySafeRWLock EffectiveWantedStateLock;
	mutable EDataLayerRuntimeState EffectiveWantedState;
	mutable int32 EffectiveWantedStateEpoch;

	mutable uint64 ProvenanceCellTransitionId = 0;
	mutable uint8 ProvenanceCellTransitionTarget = static_cast<uint8>(EWorldPartitionRuntimeCellState::Unloaded);
...
public:
	...
	inline FBox GetStreamingBounds() const { return bIsCustomHLODPlaceholderCell ? RuntimeCellData->GetCellBounds() : RuntimeCellData->GetStreamingBounds(); }
...
[[nodiscard]] constexpr bool operator == (EWorldPartitionRuntimeCellState a, EDataLayerRuntimeState b)
{
	return static_cast<uint8>(a) == static_cast<uint8>(b);
}

[[nodiscard]] constexpr bool operator == (EDataLayerRuntimeState a, EWorldPartitionRuntimeCellState b)
{
	return static_cast<uint8>(a) == static_cast<uint8>(b);
}
