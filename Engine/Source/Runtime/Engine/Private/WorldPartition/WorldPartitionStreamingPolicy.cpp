...
namespace
{
	enum class ECellTransitionAction : uint8
	{
		Unknown,
		Load,
		Activate,
	...
	enum class ECellTransitionBlockReason : uint8
	{
		None,
		StreamingDisabled,
		BudgetExhausted,
	...
	uint32 PackCellTransitionDetail(
		const UWorldPartitionRuntimeCell* Cell,
		EWorldPartitionRuntimeCellState PriorState,
		EWorldPartitionRuntimeCellState TargetState,
		EWorldPartitionRuntimeCellState ObservedState,
		ECellTransitionAction Action,
		uint8 LevelStreamingState = 0xff,
		ECellTransitionBlockReason BlockReason = ECellTransitionBlockReason::None)
	{
		uint32 PackedValue =
			(static_cast<uint32>(PriorState) & 0x3u) |
			((static_cast<uint32>(TargetState) & 0x3u) << 2) |
			((static_cast<uint32>(ObservedState) & 0x3u) << 4) |
			((static_cast<uint32>(Action) & 0x7u) << 6) |
			(static_cast<uint32>(LevelStreamingState) << 16) |
			((static_cast<uint32>(BlockReason) & 0xfu) << 24);
		if (Cell->IsAlwaysLoaded())
		{
			PackedValue |= 1u << 9;
		}
		if (Cell->IsSpatiallyLoaded())
		{
			PackedValue |= 1u << 10;
		}
	...
	void RecordCellTransitionEvent(
		...
		ECellTransitionBlockReason BlockReason = ECellTransitionBlockReason::None)
	{
		UE::RHI::ResourceProvenance::RecordCellTransition(
			Operation,
			TransitionId,
			Cell,
			PackCellTransitionDetail(Cell, PriorState, TargetState, ObservedState, Action, LevelStreamingState, BlockReason),
			Text,
			UE::RHI::ResourceProvenance::CaptureCallerAddress());
	}

	FString BuildCellTransitionText(const UWorldPartitionRuntimeCell* Cell)
	{
		FString DataLayers;
		for (const FName& DataLayer : Cell->GetDataLayers())
		{
			...
			DataLayers += DataLayer.ToString();
		}
		return FString::Printf(
			TEXT("cell_guid=%s cell=%s path=%s package=%s external_data_layer=%s data_layers=%s"),
			*Cell->GetGuid().ToString(),
			*Cell->GetDebugName(),
			*Cell->GetPathName(),
			*Cell->GetLevelPackageName().ToString(),
			*Cell->GetExternalDataLayer().ToString(),
			*DataLayers);
	}

	uint64 BeginCellTransition(
		const UWorldPartitionRuntimeCell* Cell,
		EWorldPartitionRuntimeCellState TargetState,
		ECellTransitionAction Action)
	{
		const EWorldPartitionRuntimeCellState PriorState = Cell->GetCurrentState();
		const uint64 SupersededId = Cell->GetProvenanceCellTransitionId();
		if (SupersededId != 0)
		{
			RecordCellTransitionEvent(
				UE::RHI::ResourceProvenance::EOperation::CellTransitionSuperseded,
				SupersededId,
				Cell,
				PriorState,
				Cell->GetProvenanceCellTransitionTarget(),
				PriorState,
				Action,
				TEXT("reason=new_request"));
			Cell->ClearProvenanceCellTransition(SupersededId);
		}

		const uint64 TransitionId = UE::RHI::ResourceProvenance::AllocateCellTransitionId();
		if (TransitionId != 0)
		{
			Cell->SetProvenanceCellTransition(TransitionId, TargetState);
			const FString Text = BuildCellTransitionText(Cell);
			RecordCellTransitionEvent(
				UE::RHI::ResourceProvenance::EOperation::CellStateRequest,
				TransitionId,
				Cell,
				PriorState,
				TargetState,
	...
	void BlockCellTransition(
		const UWorldPartitionRuntimeCell* Cell,
		uint64 TransitionId,
		EWorldPartitionRuntimeCellState PriorState,
		EWorldPartitionRuntimeCellState TargetState,
		ECellTransitionAction Action,
		ECellTransitionBlockReason Reason)
	{
		RecordCellTransitionEvent(
			UE::RHI::ResourceProvenance::EOperation::CellStateBlocked,
			TransitionId,
			Cell,
			PriorState,
			TargetState,
			...
			0xff,
			Reason);
		Cell->ClearProvenanceCellTransition(TransitionId);
	}
}
...
void UWorldPartitionStreamingPolicy::UpdateStreamingSources(bool bCanOptimizeUpdate)
{
	...
	{
		// Streaming Source Hash doesn't take velocity into account, we still need to update the streaming sources velocity
		WorldPartitionSubsystem->UpdateStreamingSourcesVelocity(WorldPartition, CurrentState.StreamingSources);
		return;
	}
...
#if WITH_EDITOR
		HashBuilder << GetWorld()->GetSubsystem<UWorldPartitionSubsystem>()->GetDynamicLoadingRangeScales().LinearScale;
#endif // WITH_EDITOR
		...
		if (!GServerDisallowStreamingOutDataLayersString.IsEmpty())
		{
			...
			if (const UDataLayerManager* DataLayerManager = WorldPartition->GetDataLayerManager())
			{
				for (const FString& DataLayerAssetName : AllDLAssetsStrings)
				{
					...
					{
						ServerDisallowStreamingOutDataLayers.Add(DataLayerInstance->GetFName());
					}
				}
			}
		}
...
void UWorldPartitionStreamingPolicy::UpdateStreamingStateInternal(const FUpdateStreamingStateParams& InParams, FWorldPartitionUpdateStreamingTargetState& OutTargetState)
{
...
#if WITH_EDITOR
		, InParams.DynamicLoadingRangeScales
#endif // WITH_EDITOR
	);
	...
	auto ShouldWaitForClientVisibility = [&InParams, &OutTargetState, &ServerClientsVisibleLevelNames](const UWorldPartitionRuntimeCell* Cell)
	{
		...
		if (Cell->ShouldServerWaitForClientLevelVisibility())
		{
			if (ULevel* Level = Cell->GetLevel())
			{
				...
				{
					UE_CLOGF(OutTargetState.bUpdateServerEpoch, LogWorldPartition, Verbose, "Server epoch update delayed by client visibility");
					OutTargetState.bUpdateServerEpoch = false;
					return true;
				}
			}
	...
	{
		UE_LOGF(LogWorldPartition, Verbose, "UWorldPartitionStreamingPolicy: CellsToActivate(%d), CellsToLoad(%d), CellsToUnload(%d)", OutTargetState.ToActivateCells.Num(), OutTargetState.ToLoadCells.Num(), OutTargetState.ToUnloadCells.Num());
		for (int i = 0; i < InParams.CurrentState.StreamingSources.Num(); ++i)
		{
			...
			UE_LOGF(LogWorldPartition, Verbose, "UWorldPartitionStreamingPolicy: Sources[%d] = %ls,%ls", i, *ViewLocation.ToString(), *ViewRotation.ToString());
		}
	});

#if !UE_BUILD_SHIPPING || UE_TRACE_WORLD_STREAMING_PRIORITY_ENABLED
	{
		TArray<TPair<FGuid, float>>* PriorityTraceData = nullptr;
	#if UE_TRACE_WORLD_STREAMING_PRIORITY_ENABLED
		if (InParams.bShouldTraceContainerPriorities)
		{
			PriorityTraceData = &OutTargetState.DeferredPriorityTraceData;
		}
	#endif
		UWorldPartitionStreamingPolicy::UpdateDebugCellsStreamingPriority(InParams.FrameActivateCells, InParams.FrameLoadCells, InParams.bShouldMergeStreamingSourceInfo, PriorityTraceData);
	}
#endif
...
void UWorldPartitionStreamingPolicy::PostUpdateStreamingStateInternal_GameThread(FWorldPartitionUpdateStreamingTargetState& InOutTargetState)
{
	...
	if (bIsServer)
	{
		...
		if (InOutTargetState.bUpdateServerEpoch)
		{
			ServerStreamingStateEpoch = WorldPartition->GetStreamingStateEpoch();
			ServerStreamingEnabledEpoch = ComputeServerStreamingEnabledEpoch();
			UE_LOGF(LogWorldPartition, Verbose, "Server epoch updated");
		}
		else
		{
	...
	// Emit deferred priority trace data (captured during UpdateStreamingStateInternal)
	UE_TRACE_WP_STREAMING_CONTAINER_PRIORITIES(GetWorld(), InOutTargetState.DeferredPriorityTraceData);
}
...
#if !UE_BUILD_SHIPPING || UE_TRACE_WORLD_STREAMING_PRIORITY_ENABLED
void UWorldPartitionStreamingPolicy::UpdateDebugCellsStreamingPriority(const TSet<const UWorldPartitionRuntimeCell*>& InActivateStreamingCells, const TSet<const UWorldPartitionRuntimeCell*>& InLoadStreamingCells, bool bInShouldMergeStreamingSourceInfo, TArray<TPair<FGuid, float>>* OutTraceData)
{
	...
	const bool bIsDebugStreamingPriorityShown = FWorldPartitionDebugHelper::IsRuntimeSpatialHashCellStreamingPriorityShown();
	if (bIsDebugStreamingPriorityShown || OutTraceData)
	{
		...
		{
			OutTraceData->Reset();
			OutTraceData->Reserve(CellCount);
		}

		int32 CellPrio = 0;
		for (const UWorldPartitionRuntimeCell* SortedCell : Cells)
		{
			const float NormalizedPriority = float(CellPrio++) / CellCount;
			if (bIsDebugStreamingPriorityShown)
			{
				const_cast<UWorldPartitionRuntimeCell*>(SortedCell)->SetDebugStreamingPriority(NormalizedPriority);
			}
			if (OutTraceData)
			{
				OutTraceData->Emplace(SortedCell->GetGuid(), NormalizedPriority);
			}
		}
	}
}
...
void UWorldPartitionStreamingPolicy::UpdateStreamingPerformance(EWorldPartitionStreamingPerformance NewStreamingPerformance, bool bBlockOnSlowStreaming)
{
	...
	if (WorldPartition->RuntimeHash)
	{
		...
		{
			UE_LOGF(LogWorldPartition, Log, "Streaming performance changed: %ls -> %ls",
...
void UWorldPartitionStreamingPolicy::SetCellStateToLoaded(const UWorldPartitionRuntimeCell* InCell, int32& InOutMaxCellsToLoad)
{
	...
	const EWorldPartitionRuntimeCellState PriorState = InCell->GetCurrentState();
	const bool bIsDeactivation = CurrentState.ActivatedCells.Contains(InCell);
#if RHI_RESOURCE_PROVENANCE_ENABLED
	const ECellTransitionAction Action = bIsDeactivation ? ECellTransitionAction::Deactivate : ECellTransitionAction::Load;
	const uint64 TransitionId = BeginCellTransition(InCell, EWorldPartitionRuntimeCellState::Loaded, Action);
#endif
...
#if RHI_RESOURCE_PROVENANCE_ENABLED
		RecordCellTransitionEvent(
			UE::RHI::ResourceProvenance::EOperation::CellStateAccepted,
			TransitionId,
			InCell,
			PriorState,
	...
	else if (WorldPartition->IsStreamingInEnabled())
	{
		...
		{
			BlockCellTransition(InCell, TransitionId, PriorState, EWorldPartitionRuntimeCellState::Loaded, Action, ECellTransitionBlockReason::BudgetExhausted);
		}
#endif
	}
#if RHI_RESOURCE_PROVENANCE_ENABLED
	...
	{
		BlockCellTransition(InCell, TransitionId, PriorState, EWorldPartitionRuntimeCellState::Loaded, Action, ECellTransitionBlockReason::StreamingDisabled);
	}
#endif

	if (bLoadCell)
	{
		UE_LOGF(LogWorldPartition, Verbose, "UWorldPartitionStreamingPolicy::SetCellStateToLoaded %ls", *InCell->GetName());
...
void UWorldPartitionStreamingPolicy::SetCellStateToActivated(const UWorldPartitionRuntimeCell* InCell, int32& InOutMaxCellsToLoad)
{
	...
	const EWorldPartitionRuntimeCellState PriorState = InCell->GetCurrentState();
#if RHI_RESOURCE_PROVENANCE_ENABLED
	const uint64 TransitionId = BeginCellTransition(InCell, EWorldPartitionRuntimeCellState::Activated, ECellTransitionAction::Activate);
#endif

	if (!WorldPartition->IsStreamingInEnabled())
	{
#if RHI_RESOURCE_PROVENANCE_ENABLED
		BlockCellTransition(InCell, TransitionId, PriorState, EWorldPartitionRuntimeCellState::Activated, ECellTransitionAction::Activate, ECellTransitionBlockReason::StreamingDisabled);
#endif
		return;
	}
	...
	if (bActivateCell)
	{
		...
		RecordCellTransitionEvent(
			...
			PriorState,
			ECellTransitionAction::Activate);
#endif
		UE_LOGF(LogWorldPartition, Verbose, "UWorldPartitionStreamingPolicy::SetCellStateToActivated %ls", *InCell->GetName());
	...
	{
		BlockCellTransition(InCell, TransitionId, PriorState, EWorldPartitionRuntimeCellState::Activated, ECellTransitionAction::Activate, ECellTransitionBlockReason::BudgetExhausted);
	}
#endif
}
...
void UWorldPartitionStreamingPolicy::SetCellsStateToUnloaded(const TArray<TObjectPtr<const UWorldPartitionRuntimeCell>>& InToUnloadCells)
{
	...
	for (const UWorldPartitionRuntimeCell* Cell : InToUnloadCells)
	{
		const EWorldPartitionRuntimeCellState PriorState = Cell->GetCurrentState();
#if RHI_RESOURCE_PROVENANCE_ENABLED
		const uint64 TransitionId = BeginCellTransition(Cell, EWorldPartitionRuntimeCellState::Unloaded, ECellTransitionAction::Unload);
#endif
		if (Cell->CanUnload())
		{
#if RHI_RESOURCE_PROVENANCE_ENABLED
			RecordCellTransitionEvent(
				...
				PriorState,
				ECellTransitionAction::Unload);
#endif
			UE_LOGF(LogWorldPartition, Verbose, "UWorldPartitionStreamingPolicy::UnloadCells %ls", *Cell->GetName());
			Cell->Unload();
		...
		{
			BlockCellTransition(Cell, TransitionId, PriorState, EWorldPartitionRuntimeCellState::Unloaded, ECellTransitionAction::Unload, ECellTransitionBlockReason::CannotUnload);
		}
#endif
	}
}
...
bool UWorldPartitionStreamingPolicy::IsStreamingCompleted(const TArray<FWorldPartitionStreamingSource>* InStreamingSources) const
{
	...
	// Non-spatial cells were already queried above; skip them for each per-source spatial query.
	check(WorldPartition->RuntimeHash->bQueryNonSpatialCells); // Flag future re-entrancy issues
	TGuardValue<bool> QueryNonSpatialCells(WorldPartition->RuntimeHash->bQueryNonSpatialCells, false);
	...
	for (const FWorldPartitionStreamingSource& StreamingSource : QueriedStreamingSources)
	{
		...
		QuerySource.bDataLayersOnly = false;
		// Always include active Data Layers even when testing loaded because we are querrying in non-exact state
		QuerySource.DataLayers = DataLayerManager->GetEffectiveActiveDataLayerNames().Array();
		if (StreamingSource.TargetState == EStreamingSourceTargetState::Loaded)
		{
			QuerySource.DataLayers.Append(DataLayerManager->GetEffectiveLoadedDataLayerNames().Array());
		}
...
bool UWorldPartitionStreamingPolicy::IsStreamingCompleted(EWorldPartitionRuntimeCellState QueryState, const TArray<FWorldPartitionStreamingQuerySource>& QuerySources, bool bExactState) const
{
	...
	{
		// Verify that the state of the query data layers are in the query target state (or greater if not querying for exact state)
		bool bAreQueryDLsInTargetState = true;
		for (const FName& QueryDataLayer : QuerySource.DataLayers)
		{
			EDataLayerRuntimeState QueryDataLayerState = StreamingContext.GetDataLayerEffectiveRuntimeState(QueryDataLayer);
			if ((QueryDataLayerState != QueryState) && (bExactState || QueryDataLayerState < QueryState))
			{
				bAreQueryDLsInTargetState = false;
				break;
			}
		}

		WorldPartition->RuntimeHash->ForEachStreamingCellsQuery(QuerySource, [QuerySource, QueryState, bExactState, bIsHLODEnabled, DataLayerManager, bAreQueryDLsInTargetState, &StreamingContext, &bResult](const UWorldPartitionRuntimeCell* Cell)
		{
			...
			if (CellState != QueryState)
			{
				...
				// Don't consider Custom HLOD Placeholder cells as they have no level streaming object
				if (!bSkipCell && Cell->GetIsCustomHLODPlaceholderCell())
				{
					bSkipCell = true;
				}
				...
				{
					if (!bAreQueryDLsInTargetState)
					{
						bResult = false;
						return false;
					}

					// Because we already validated that the queried data layers are in the proper effective state
					// if the cell wanted state is not in the proper queried state, it's because of unquerried cell data layer states.
					// In this case, we must ignore the cell.
					const EDataLayerRuntimeState CellWantedState = Cell->GetCellEffectiveWantedState(StreamingContext);
					bSkipCell = ((CellWantedState != QueryState) && (bExactState || CellWantedState < QueryState));
				}
...
void UWorldPartitionStreamingPolicy::OnCellShown(const UWorldPartitionRuntimeCell* InCell)
{
	...
	if (TransitionId != 0)
	{
		...
		if (bCompleted)
		{
			InCell->ClearProvenanceCellTransition(TransitionId);
		}
	}
#endif
}
...
#if UE_TRACE_WORLD_STREAMING_PRIORITY_ENABLED
	DeferredPriorityTraceData.Reset();
#endif
}
...
#if UE_TRACE_WORLD_STREAMING_PRIORITY_ENABLED
	, bShouldTraceContainerPriorities(UWorldStreamingTraceSubsystem::ShouldTraceContainerPriorityThisFrame(InPolicy->GetWorld()))
#endif
...
#if WITH_EDITOR
	, DynamicLoadingRangeScales(
		[InPolicy]()
		{
			const UWorldPartitionSubsystem* Subsystem = UWorld::GetSubsystem<UWorldPartitionSubsystem>(InPolicy->GetWorld());
			return (Subsystem) ? Subsystem->GetDynamicLoadingRangeScales() : FDynamicLoadingRangeScales{};
		}())
#endif // WITH_EDITOR
