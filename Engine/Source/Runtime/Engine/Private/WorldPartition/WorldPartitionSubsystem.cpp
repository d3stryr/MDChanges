...
int32 GLevelStreamingContinuouslyIncrementalGCWhileActorsPendingPurgeForWP = 0;	// Disabled by default
static FAutoConsoleVariableRef CVarGLevelStreamingContinuouslyIncrementalGCWhileActorsPendingPurgeForWP(
	TEXT("wp.Runtime.LevelStreamingContinuouslyIncrementalGCWhileActorsPendingPurgeForWP"),
	GLevelStreamingContinuouslyIncrementalGCWhileActorsPendingPurgeForWP,
	TEXT("Force a GC update when there's more than the number of specified Actors (from levels) pending purge."),
	ECVF_Default
);
...
TMap<FActorContainerID, TSet<FGuid>> UWorldPartitionSubsystem::GetFilteredActorsPerContainer(const FActorContainerID& InContainerID, const FString& InWorldPackage, const FWorldPartitionActorFilter& InActorFilter, EWorldPartitionActorFilterType InFilterTypes)
{
	...
	TFunction<void(const FActorContainerID&, const UActorDescContainer*, TSet<FName>&)> ProcessContainers = [&FindOrRegisterContainer, &DataLayerFiltersPerContainer, &FilteredActors, &ProcessContainers, &InFilterTypes](const FActorContainerID& InContainerID, const UActorDescContainer* InContainer, TSet<FName>& InOutVisitedPackageStack)
	{
		...
		{
			if (ActorDescIt->GetDataLayers().Num() > 0)
			{
...
{
	static UClass* WorldPartitionConvertCommandletClass = FindObject<UClass>(nullptr, TEXT("/Script/UnrealEd.WorldPartitionConvertCommandlet"), EFindObjectFlags::ExactClass);
...
void UWorldPartitionSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	{
#if WITH_EDITOR
		FWorldDelegates::OnStartGameInstance.AddUObject(this, &UWorldPartitionSubsystem::OnStartGameInstance);
#endif // WITH_EDITOR
...
#if WITH_EDITOR
void UWorldPartitionSubsystem::OnStartGameInstance(UGameInstance* GameInstance)
{
	InitDynamicLoadingRangeScale();
}
#endif // WITH_EDITOR
...
void UWorldPartitionSubsystem::OnWorldPartitionInitialized(UWorldPartition* InWorldPartition)
{
	if (RegisteredWorldPartitions.IsEmpty())
	{
		...
		if (GetWorld()->IsGameWorld())
		{
			...
			LevelStreamingContinuouslyIncrementalGCWhileActorsPendingPurge = GLevelStreamingContinuouslyIncrementalGCWhileActorsPendingPurge;
			...
			GLevelStreamingContinuouslyIncrementalGCWhileActorsPendingPurge = GLevelStreamingContinuouslyIncrementalGCWhileActorsPendingPurgeForWP;
...
void UWorldPartitionSubsystem::OnWorldPartitionUninitialized(UWorldPartition* InWorldPartition)
{
	...
	if (OwningWorld->IsGameWorld())
	{
		...
		if (InWorldPartition->GetTypedOuter<UWorld>() != OwningWorld)
		{
			...
			for (ULevelStreaming* StreamingLevel : OwningWorld->GetStreamingLevels())
			{
				...
				if (WorldPartitionStreamingLevel && 
					(WorldPartitionStreamingLevel->GetOuterWorldPartition() == WorldPartition) && 
					(WorldPartitionStreamingLevel->GetLevelStreamingState() != ELevelStreamingState::FailedToLoad))
				{
	...
	if (RegisteredWorldPartitions.IsEmpty())
	{
		if (GetWorld()->IsGameWorld())
		{
			...
			GLevelStreamingContinuouslyIncrementalGCWhileActorsPendingPurge = LevelStreamingContinuouslyIncrementalGCWhileActorsPendingPurge;
...
void UWorldPartitionSubsystem::OnLevelStreamingStateChanged(UWorld* InWorld, const ULevelStreaming* InStreamingLevel, ULevel* LevelIfLoaded, ELevelStreamingState PreviousState, ELevelStreamingState NewState)
{
	...
	UE_TRACE_WP_STREAMING_CONTAINER_STATE(InWorld, InStreamingLevel, NewState);

	UpdateLoadingAndPendingLoadStreamingLevels(InStreamingLevel);

#if RHI_RESOURCE_PROVENANCE_ENABLED
	if (const UWorldPartitionLevelStreamingDynamic* DiagnosticStreamingLevel = Cast<const UWorldPartitionLevelStreamingDynamic>(InStreamingLevel))
	{
		if (const UWorldPartitionRuntimeCell* RuntimeCell = DiagnosticStreamingLevel->GetWorldPartitionRuntimeCell())
		{
			const uint64 TransitionId = RuntimeCell->GetProvenanceCellTransitionId();
			if (TransitionId != 0)
			{
				const EWorldPartitionRuntimeCellState TargetState = RuntimeCell->GetProvenanceCellTransitionTarget();
				const auto ToCellState = [RuntimeCell](ELevelStreamingState State)
				{
					switch (State)
					{
					...
					default:
						return RuntimeCell->GetCurrentState();
					}
				};

				const EWorldPartitionRuntimeCellState PriorCellState = ToCellState(PreviousState);
				const EWorldPartitionRuntimeCellState ObservedCellState = ToCellState(NewState);
				const bool bFailed = NewState == ELevelStreamingState::FailedToLoad;
				const bool bCompleted =
					(TargetState == EWorldPartitionRuntimeCellState::Unloaded &&
						(NewState == ELevelStreamingState::Removed || NewState == ELevelStreamingState::Unloaded)) ||
					(TargetState == EWorldPartitionRuntimeCellState::Loaded && NewState == ELevelStreamingState::LoadedNotVisible) ||
					(TargetState == EWorldPartitionRuntimeCellState::Activated && NewState == ELevelStreamingState::LoadedVisible);
				const UE::RHI::ResourceProvenance::EOperation Operation = bFailed
					? UE::RHI::ResourceProvenance::EOperation::CellStateBlocked
					: bCompleted
						? UE::RHI::ResourceProvenance::EOperation::CellStateCompleted
						: UE::RHI::ResourceProvenance::EOperation::CellStateProgress;

				uint32 PackedValue =
					(static_cast<uint32>(PriorCellState) & 0x3u) |
					((static_cast<uint32>(TargetState) & 0x3u) << 2) |
					((static_cast<uint32>(ObservedCellState) & 0x3u) << 4) |
					(7u << 6) |
					(static_cast<uint32>(NewState) << 16) |
					((bFailed ? 4u : 0u) << 24);
				if (RuntimeCell->IsAlwaysLoaded())
				{
					PackedValue |= 1u << 9;
				}
				if (RuntimeCell->IsSpatiallyLoaded())
				{
					PackedValue |= 1u << 10;
				}
				if (RuntimeCell->HasDataLayers())
				{
					PackedValue |= 1u << 11;
				}
				const FString Text = FString::Printf(
					TEXT("level=%s previous=%s current=%s"),
					*InStreamingLevel->GetPathName(),
				...
				if (bFailed || bCompleted)
				{
					RuntimeCell->ClearProvenanceCellTransition(TransitionId);
				}
			}
		}
	}
...
void UWorldPartitionSubsystem::RegisterStreamingSourceProvider(IWorldPartitionStreamingSourceProvider* StreamingSource)
{
	...
	UE_CLOGF(bIsAlreadyInSet, LogWorldPartition, Warning, "Streaming source provider already registered.");
}
...
#if WITH_EDITOR
static const FName NAME_SIEStreamingSource("SIE");
#endif
...
void UWorldPartitionSubsystem::UpdateStreamingSources()
{
	...
	for (FWorldPartitionStreamingSource& StreamingSource : StreamingSources)
	{
		...
		UE_TRACE_WP_STREAMING_SOURCE_STATE(World, StreamingSource);
	}

	UE_TRACE_WP_STREAMING_SOURCE_END_UPDATES(World);
...
void UWorldPartitionSubsystem::UpdateStreamingSourcesVelocity(const UWorldPartition* InWorldPartition, TArray<FWorldPartitionStreamingSource>& InOutStreamingSources) const
{
	if (InOutStreamingSources.Num())
	{
		const FTransform WorldToLocal = InWorldPartition->GetInstanceTransform().Inverse();
		for (FWorldPartitionStreamingSource& StreamingSourceToUpdate : InOutStreamingSources)
		{
			// Here we can't assume that StreamingSources matches InOutStreamingSources, so we find the streaming sources based on its name
			if (const FWorldPartitionStreamingSource* StreamingSource = StreamingSources.FindByPredicate([Name = StreamingSourceToUpdate.Name](const FWorldPartitionStreamingSource& Source) { return Source.Name == Name; }))
			{
				StreamingSourceToUpdate.Velocity = WorldToLocal.TransformVector(StreamingSource->Velocity);
			}
		}
	}
}
...
void UWorldPartitionSubsystem::UpdateStreamingStateInternal(const UWorld* InWorld, UWorldPartition* InWorldPartition)
{
	...
	auto GetRegisteredWorldPartitionsCopy = [&RegisteredWorldPartitionsCopy, InWorldPartition, WorldPartitionSubsystem]()
	{
		if (RegisteredWorldPartitionsCopy.IsEmpty())
		{
			...
			RegisteredWorldPartitionsCopy = InWorldPartition ? TArray<TObjectPtr<UWorldPartition>>({ InWorldPartition }) : (WorldPartitionSubsystem) ? WorldPartitionSubsystem->RegisteredWorldPartitions : TArray<TObjectPtr<UWorldPartition>>{};
		}
	...
	const int32 WorldPartitionUpdateCount = InWorldPartition ? 1 : (WorldPartitionSubsystem) ? WorldPartitionSubsystem->RegisteredWorldPartitions.Num() : 0;
	...
	if (WorldPartitionSubsystem)
	{
		WorldPartitionSubsystem->UpdateDynamicLoadingRangeScale();
	}
#endif // WITH_EDITOR
	...
	if (bIncrementalUpdate)
	{
		...
		if (WorldPartitionSubsystem && !WorldPartitionSubsystem->IncrementalUpdateStreamingState())
		{
			return;
		}
	}
...
void UWorldPartitionSubsystem::Draw(UCanvas* Canvas, class APlayerController* PC)
{
	...
	if (GDrawStreamingSources || GDrawRuntimeHash2D)
	{
		...
		{
			FString Title("Streaming Sources");
...
namespace UE
{
	namespace Private
	{
		void AssignLinearScale(FDynamicLoadingRangeScales& Scales, float Linear)
		{
			Scales.LinearScale = Linear;
			Scales.Scale2D = FMath::Sqrt(Scales.LinearScale);
			Scales.Scale3D = FMath::Pow(Scales.LinearScale, 1.f / 3.f);
		}
	}
}

void UWorldPartitionSubsystem::InitDynamicLoadingRangeScale()
{
	if (GetWorld()->GetWorldSettings()->GetDynamicLoadingRangeScalingEnable())
	{
		FDynamicLoadingRangeScaling& ScaleSettings = GetWorld()->GetWorldSettings()->DynamicLoadingRangeScaling;
		UE::Private::AssignLinearScale(LoadingRangeScales, FMath::Clamp(ScaleSettings.InitialDynamicLoadingRangeScaling, 0.f, 100.f) * 0.01f);
		DynamicLoadingRangeScaleStartTime = GetWorld()->GetTimeSeconds();
	}
	else
	{
		UE::Private::AssignLinearScale(LoadingRangeScales, 1.f);
	}
}

void UWorldPartitionSubsystem::UpdateDynamicLoadingRangeScale()
{
	if (LoadingRangeScales.LinearScale < 1.f) // Can skip the update if the scale is at 1.f already
	{
		if (GetWorld()->GetWorldSettings()->GetDynamicLoadingRangeScalingEnable())
		{
			FDynamicLoadingRangeScaling& ScaleSettings = GetWorld()->GetWorldSettings()->DynamicLoadingRangeScaling;
			// ScaleSettings.InitialDynamicLoadingRangeScaling is a percentage value
			float InitialScale = FMath::Clamp(ScaleSettings.InitialDynamicLoadingRangeScaling * 0.01f, 0.f, 1.f);
			if (GetWorld()->GetIsInBlockTillLevelStreamingCompleted() && (LoadingRangeScales.LinearScale == InitialScale))
			{
				// Do not interpolate as long as we are still loading the levels, and use the initial loading range
				DynamicLoadingRangeScaleStartTime = GetWorld()->GetTimeSeconds(); // StartTime is the time when initial level loading is complete, so update the time
			}
			else
			{
				// Start interpolating
				float InterpolationTime = GetWorld()->GetTimeSeconds() - DynamicLoadingRangeScaleStartTime;
				float Delay = FMath::Max(ScaleSettings.DynamicLoadingRangeScalingDelay, 0.0f);
				float Duration = FMath::Max(ScaleSettings.DynamicLoadingRangeScalingDuration, 0.0f);

				float EffectiveTime = InterpolationTime - Delay;
				float Progress = (EffectiveTime < 0.f) ? 0.f : (Duration == 0.f) ? 1.f : FMath::Min(EffectiveTime / Duration, 1.f);
				UE::Private::AssignLinearScale(LoadingRangeScales, Progress * (1.f - InitialScale) + InitialScale);
			}
		}
		else
		{
			// Reset scale to 1.f if user disables the feature during runtime
			UE::Private::AssignLinearScale(LoadingRangeScales, 1.f);
		}
	}
}
...
FVector FStreamingSourceVelocity::GetAverageVelocity(const FVector& NewPosition, double CurrentTime)
{
	...
	if (DeltaSeconds <= 0.0)
	{
		UE_LOGF(LogWorldPartition, Verbose, "Detected Invalid Delta Time: %ls", *SourceName.ToString());
	...
	else
	{
		...
		if (bNewSource)
		{
			UE_LOGF(LogWorldPartition, Verbose, "New Streaming Source: %ls -> Position: %ls", *SourceName.ToString(), *NewPosition.ToString());
			AvgVelocity = FVector::Zero();
		}
		else if (Distance > TeleportDistance)
		{
			UE_LOGF(LogWorldPartition, Verbose, "Detected Streaming Source Teleport: %ls -> Last Position: %ls -> New Position: %ls", *SourceName.ToString(), *LastPosition.ToString(), *NewPosition.ToString());
			AvgVelocity = FVector::Zero();
		}
		else if  (DeltaSeconds > MaxDeltaSeconds)
		{
			UE_LOGF(LogWorldPartition, Verbose, "Detected Inactive Streaming Source: %ls -> Last Position: %ls -> New Position: %ls", *SourceName.ToString(), *LastPosition.ToString(), *NewPosition.ToString());
			AvgVelocity = FVector::Zero();
		}
		else
		{
