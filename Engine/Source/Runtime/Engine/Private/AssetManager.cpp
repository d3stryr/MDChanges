...
#if UE_ASSETMANAGER_SMALL_LOADSTATE

/** Structure defining the current loading state of an asset */
struct FPrimaryAssetLoadState
{
private:
	// Note: this implementation reduces memory by moving as much of the state as possible into a single buffer.
	// If the state is empty, this buffer is not allocated so we don't pay the memory for it.
	// Layout is [ (TSharedPtr<FStreamableHandle>) | (NumBundleNames * sizeof(FName)) | (optional FAssetManagerDownloadParams)]

	/** The storage buffer */
	uint8* Storage = nullptr;
	/** Number of bundles in the storage buffer */
	uint16 NumBundleNames = 0;
	/** If true, the buffer contains download params */
	bool bWithDownloadParams = 0;

	constexpr static SIZE_T GetBundleNamesOffset()
	{ 
		return Align(sizeof(TSharedPtr<FStreamableHandle>), alignof(FName));
	}

	static SIZE_T GetDownloadParamsOffset(uint16 InNumBundles)
	{
		return Align(GetBundleNamesOffset() + sizeof(FName) * InNumBundles, alignof(FAssetManagerDownloadParams));
	}

	void ReallocStorage(uint16 InNumBundles, bool bInWithDownloadParams)
	{
		FMemory::Free(Storage);

		const SIZE_T StorageSize = bInWithDownloadParams
			? (GetDownloadParamsOffset(InNumBundles) + sizeof(FAssetManagerDownloadParams))
			: (GetBundleNamesOffset() + sizeof(FName) * InNumBundles);

		Storage = reinterpret_cast<uint8*>(FMemory::Malloc(StorageSize));
	}

	bool NeedsRealloc(uint16 InNumBundles, bool bInWithDownloadParams) const
	{
		return !Storage || InNumBundles != NumBundleNames || bInWithDownloadParams != bWithDownloadParams;
	}

	void DestroyStoredItems()
	{
		check(Storage);
		DestructItem(GetHandleInternal());
		DestructItems(GetBundleNamesInternal(), NumBundleNames);
		if (FAssetManagerDownloadParams* DownloadParams = GetDownloadParamsInternal())
		{
			DestructItem(DownloadParams);
		}
	}
	...
	TSharedPtr<FStreamableHandle>* GetHandleInternal()
	{
		check(Storage);
		return UE_LAUNDER(reinterpret_cast<TSharedPtr<FStreamableHandle>*>(Storage));
	}

	const TSharedPtr<FStreamableHandle>* GetHandleInternal() const
	{
		return const_cast<FPrimaryAssetLoadState*>(this)->GetHandleInternal();
	}

	/** The set of bundles to be loaded by the handle */
	FName* GetBundleNamesInternal()
	{
		check(Storage);
		return UE_LAUNDER(reinterpret_cast<FName*>(Storage + GetBundleNamesOffset()));
	}

	const FName* GetBundleNamesInternal() const
	{
		return const_cast<FPrimaryAssetLoadState*>(this)->GetBundleNamesInternal();
	}

	/** Params for installing assets before loading */
	FAssetManagerDownloadParams* GetDownloadParamsInternal()
	{
		if (bWithDownloadParams)
		{
			check(Storage);
			return UE_LAUNDER(reinterpret_cast<FAssetManagerDownloadParams*>(Storage + GetDownloadParamsOffset(NumBundleNames)));
		}

		return nullptr;
	}

	const FAssetManagerDownloadParams* GetDownloadParamsInternal() const
	{
		return const_cast<FPrimaryAssetLoadState*>(this)->GetDownloadParamsInternal();
	}

	inline static const TSharedPtr<FStreamableHandle> InvalidHandle = {};
...
public:
	...
	FPrimaryAssetLoadState(FPrimaryAssetLoadState&& Other) 
		...
		, NumBundleNames(Other.NumBundleNames)
		, bWithDownloadParams(Other.bWithDownloadParams)
	{
		Other.Storage = nullptr;
		Other.NumBundleNames = 0;
		Other.bWithDownloadParams = false;
	}
	...
	{
		if (NumBundleNames == InBundleNames.Num() && bWithDownloadParams == InDownloadParams.IsSet())
		{
			// bWithDownloadParams can only be true if storage is allocated
			if (bWithDownloadParams && *GetDownloadParamsInternal() != InDownloadParams)
			{
				return false;
			}

			return Algo::Compare(InBundleNames, GetBundleNames());
		}
	...
	{
		const uint16 InNumBundleNames = IntCastChecked<uint16>(InBundleNames.Num());

		if (NeedsRealloc(InNumBundleNames, InDownloadParams.IsSet()))
		{
			if (Storage)
			{
				DestroyStoredItems();
			}
			ReallocStorage(InNumBundleNames, InDownloadParams.IsSet());
			NumBundleNames = InNumBundleNames;
			bWithDownloadParams = InDownloadParams.IsSet();

			new(Storage) TSharedPtr<FStreamableHandle>(MoveTemp(InHandle));

			ConstructItems<FName>(GetBundleNamesInternal(), InBundleNames.GetData(), InNumBundleNames);

			if (InDownloadParams)
			{
				new(GetDownloadParamsInternal()) FAssetManagerDownloadParams(MoveTemp(*InDownloadParams));
			}
		}
		else
		{
			*GetHandleInternal() = MoveTemp(InHandle);

			CopyAssignItems(GetBundleNamesInternal(), InBundleNames.GetData(), InNumBundleNames);

			if (InDownloadParams)
			{
				*GetDownloadParamsInternal() = MoveTemp(*InDownloadParams);
			}
		}
	}
	...
	{ 
		if (Storage)
		{
			return MakeArrayView(GetBundleNamesInternal(), NumBundleNames);
		}
		return {};
	}
	...
	bool IsValid() const 
	{
		const TSharedPtr<FStreamableHandle>& Handle = GetHandle();
...
public:
	...
	void SetState(TSharedPtr<FStreamableHandle> InHandle, TConstArrayView<FName> InBundleNames, TOptional<FAssetManagerDownloadParams> InDownloadParams)
	{
		...
		BundleNames = InBundleNames;
		DownloadParams = MoveTemp(InDownloadParams);
	}

	void MoveStateFrom(TSharedPtr<FStreamableHandle> InHandle, FPrimaryAssetLoadState&& InOther)
	{
		Handle = MoveTemp(InHandle);
		BundleNames = MoveTemp(InOther.BundleNames);
		DownloadParams = MoveTemp(InOther.DownloadParams);
	}

	const TSharedPtr<FStreamableHandle>& GetHandle() const { return Handle; }

	const TArray<FName>& GetBundleNames() const { return BundleNames; }
...
#endif // UE_ASSETMANAGER_SMALL_LOADSTATE
...
struct FPrimaryAssetTypeData
{
	...
	void RemoveAssets(TConstArrayView<FName> AssetNames, TMap<FSoftObjectPath, FPrimaryAssetId>& InOutReverseMap);
...
void FPrimaryAssetTypeData::RemoveAssets(TConstArrayView<FName> AssetNames, TMap<FSoftObjectPath, FPrimaryAssetId>& InOutReverseMap)
{
	FPrimaryAssetData ExistingAssetData;
...
const FPrimaryAssetType UAssetManager::MapType = FName("Map");
const FPrimaryAssetType UAssetManager::PrimaryAssetLabelType = FName("PrimaryAssetLabel");
const FPrimaryAssetType UAssetManager::PackageChunkType = FName("PackageChunk");
const FPrimaryAssetType UAssetManager::SuppressionType = FName("__Suppressed__");
...
static FDelayedAutoRegisterHelper GInitialBulkScanStopper(EDelayedRegisterRunPhase::StartOfEnginePreInit,
	[] { FCoreDelegates::GetOnPostEngineInit().AddLambda([] { GInitialBulkScan.StopOnce(); }); } );
...
namespace UE::AssetManager::Private
{
		...
		FStringBuilderBase& DebugValid, FStringBuilderBase& DebugInvalid)
	{
		...
		if (DebugInvalid.Len() > 0)
		{
			...
			UE_LOGF(LogAssetManager, Warning, "Requested preload of some Primary Assets failed: %ls", *DebugInvalid);
...
void UAssetManager::PostInitProperties()
{
	...
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		{
			UE_LOGF(LogAssetManager, Display, "Detected PakchunkId override config. Binding PakChunkId override delegate.");
...
UAssetManager& UAssetManager::Get()
{
	...
	{
		UE_LOGF(LogAssetManager, Fatal, "Cannot use AssetManager if no AssetManagerClassName is defined!");
		...
		if (Object->IsA(TypeData.Info.AssetBaseClassLoaded))
		{
			...
			for (const FString& ScanPath : TypeData.RealAssetScanPaths)
			{
				if (AssetPath.StartsWith(ScanPath))
				{
					...
					{
						UE_LOGF(LogAssetManager, Warning, "Found Duplicate PrimaryAssetType %ls for asset %ls which is already registered as %ls, it is not possible to have conflicting directories when bShouldManagerDetermineTypeAndName is true!", *TypeData.Info.PrimaryAssetType.ToString(), *AssetPath, *FoundType.ToString());
					}
					else
					{
...
int32 UAssetManager::SearchAssetRegistryPaths(TArray<FAssetData>& OutAssetDataList, const FAssetManagerSearchRules& Rules) const
{
	...
	ARFilter.bIncludeOnlyOnDiskAssets = !GIsEditor || IsRunningCookCommandlet(); // In editor check in memory, otherwise don't
...
int32 UAssetManager::ScanPathsForPrimaryAssets(FPrimaryAssetType PrimaryAssetType, const TArray<FString>& Paths, UClass* BaseClass, bool bHasBlueprintClasses, bool bIsEditorOnly, bool bForceSynchronousScan)
{
	...
	if (!FoundType)
	{
		UE_LOGF(LogAssetManager, VeryVerbose, "UAssetManager::ScanPathsForPrimaryAssets adding TypeData to AssetTypeMap: '%ls', (Paths=%ls%ls)",
			*PrimaryAssetType.ToString(),
			*FString::Join(TArrayView<const FString>(Paths).Left(FMath::Min(Paths.Num(), 3)), TEXT(", ")),
			(Paths.Num() > 3) ? TEXT(", ...") : TEXT(""));
	...
	if (TypeData.Info.AssetBaseClassLoaded != BaseClass || TypeData.Info.bHasBlueprintClasses != bHasBlueprintClasses || TypeData.Info.bIsEditorOnly != bIsEditorOnly)
	{
		auto CalculateEnsureMessage = [&TypeData, &Paths, BaseClass, bHasBlueprintClasses, bIsEditorOnly]()
		{
			FString Message;
			if (TypeData.Info.AssetBaseClassLoaded != BaseClass)
			{
				Message += FString::Printf(TEXT(", (BaseClass: '%s' != expected: '%s')"), *GetNameSafe(BaseClass), *GetNameSafe(TypeData.Info.AssetBaseClassLoaded.Get()));
			}

			if (TypeData.Info.bHasBlueprintClasses != bHasBlueprintClasses)
			{
				Message += FString::Printf(TEXT(", (bHasBlueprintClasses: '%d' != expected: '%d')"), bHasBlueprintClasses, TypeData.Info.bHasBlueprintClasses);
			}

			if (TypeData.Info.bIsEditorOnly != bIsEditorOnly)
			{
				Message += FString::Printf(TEXT(", (bIsEditorOnly: '%d' != expected: '%d')"), bIsEditorOnly, TypeData.Info.bIsEditorOnly);
			}

			if (Paths.Num() > 0)
			{
				// Context about where first PrimaryAssetType was set
				Message += FString::Printf(TEXT(", (Paths=%s%s)"),
					*FString::Join(TArrayView<const FString>(Paths).Left(FMath::Min(Paths.Num(), 3)), TEXT(", ")),
					(Paths.Num() > 3) ? TEXT(", ...") : TEXT(""));
			}

			Message += TEXT(". Another plugin, ini, or system has different parameters for this PrimaryAssetType name. Run with -LogCmds=\"LogAssetManager VeryVerbose\" to log more information.");

			return Message;
		};

		ensureMsgf(false, TEXT("UAssetManager::ScanPathsForPrimaryAssets TypeData parameters did not match PrimaryAssetType '%s'%s"), *TypeData.Info.PrimaryAssetType.ToString(), *CalculateEnsureMessage());
		return 0;
	}
	...
	for (FAssetData& Data : AssetDataList)
	{
			...
			PrimaryAssetId.PrimaryAssetType != PrimaryAssetType)
		{
			...
			{
				UE_LOGF(LogAssetManager, Warning, "Ignoring primary asset %ls - PrimaryAssetType %ls - invalid primary asset ID", *Data.GetObjectPathString(), *PrimaryAssetType.ToString());
			}
			else
			{
				...
				if (!IssuedWarnings.Contains(ConflictPair))
				{
					...
					UE_LOGF(LogAssetManager, Display, "%ls", *ConflictMsg);
...
void UAssetManager::RemovePrimaryAssetsForTypeInMountPoint(FPrimaryAssetType PrimaryAssetType, FStringView RootPath)
{
	TSharedRef<FPrimaryAssetTypeData>* FoundType = AssetTypeMap.Find(PrimaryAssetType);
	...
	AssetsToRemove.Reserve(TypeData.GetAssets().Num());
	for (const TPair<FName, FPrimaryAssetData>& Pair : TypeData.GetAssets())
	{
		FNameBuilder PackageName(Pair.Value.GetAssetPtr().GetLongPackageFName());
		if (PackageName.ToView().StartsWith(RootPath))
		{
			AssetsToRemove.Add(Pair.Key);
		}
	}

	TypeData.RemoveAssets(AssetsToRemove, AssetPathMap);
	OnObjectReferenceListInvalidated();
}
...
bool UAssetManager::TryUpdateCachedAssetData(const FPrimaryAssetId& PrimaryAssetId, const FAssetData& NewAssetData, bool bAllowDuplicates)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogAssetManager, Warning, "Tried to add primary asset %ls, but it is not a TopLevelAsset and so cannot be a primary asset",
		...
		{
			UE_LOGF(LogAssetManager, Warning, 
				"Found duplicate PrimaryAssetID %ls, path %ls conflicts with existing path %ls. Two different primary assets can not have the same type and name.", 
...
bool UAssetManager::AddDynamicAsset(const FPrimaryAssetId& PrimaryAssetId, const FSoftObjectPath& AssetPath, const FAssetBundleData& BundleData)
{
	...
	{
		UE_LOGF(LogAssetManager, Warning, "AddDynamicAsset on %ls called with conflicting path. Path %ls is replacing path %ls",
...
void UAssetManager::SetPrimaryAssetRulesExplicitly(FPrimaryAssetId PrimaryAssetId, const FPrimaryAssetRulesExplicitOverride& ExplicitRules)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogAssetManager, Error, "Duplicate Rule overrides found for asset %ls!", *PrimaryAssetId.ToString());
		}
...
bool UAssetManager::GetPrimaryAssetIdList(FPrimaryAssetType PrimaryAssetType, TArray<FPrimaryAssetId>& PrimaryAssetIdList, EAssetManagerFilter Filter) const
{
	...
	if (FoundType)
	{
		...
		for (const TPair<FName, FPrimaryAssetData>& Pair : TypeData.GetAssets())
		{
			if ((!(Filter & EAssetManagerFilter::UnloadedOnly)) || ((Pair.Value.CurrentState.GetBundleNames().Num() == 0) && (Pair.Value.PendingState.GetBundleNames().Num() == 0)))
			{
...
void UAssetManager::GetPrimaryAssetsInPath(FPrimaryAssetType PrimaryAssetType, FStringView Path, TArray<FPrimaryAssetId>& PrimaryAssetIdList) const
{
	const TSharedRef<FPrimaryAssetTypeData>* FoundType = AssetTypeMap.Find(PrimaryAssetType);
	if (FoundType)
	{
		...
		{
			FNameBuilder PackageName(Pair.Value.GetAssetPtr().GetLongPackageFName());
			if (PackageName.ToView().StartsWith(Path))
			{
				PrimaryAssetIdList.Add(FPrimaryAssetId(PrimaryAssetType, Pair.Key));
			}
		}
	...
	UE::FSourceLocation Location)
{
	...
	const uint64 ProvenanceCallerAddress = UE::RHI::ResourceProvenance::CaptureCallerAddress();
#endif
	for (const FPrimaryAssetId& PrimaryAssetId : AssetsToChange)
	{
		...
		if (NameData)
		{
...
#if RHI_RESOURCE_PROVENANCE_ENABLED
			if (UMaterialParameterCollection* Collection = Cast<UMaterialParameterCollection>(
				GetPrimaryAssetObject(PrimaryAssetId)))
			{
				Collection->RecordAssetManagerProvenance(
					EMPCAssetProvenancePhase::AssetManagerLoadRequest,
					FString::Printf(
						TEXT("manager=%s primary_asset=%s add_bundles=%d remove_bundles=%d remove_all=%d current_valid=%d pending_valid=%d"),
						*GetClass()->GetPathName(),
						*PrimaryAssetId.ToString(),
						AddBundles.Num(),
						RemoveBundles.Num(),
						bRemoveAllBundles ? 1 : 0,
						NameData->CurrentState.IsValid() ? 1 : 0,
						NameData->PendingState.IsValid() ? 1 : 0),
					ProvenanceCallerAddress);
			}
#endif
			...
			{
				NewBundleState = NameData->PendingState.IsValid() ? NameData->PendingState.GetBundleNames() : NameData->CurrentState.GetBundleNames();
			...
			if (NameData->PendingState.IsValid())
			{
				if (NameData->PendingState.IsSame(NewBundleState, LoadParams.DownloadParams))
				{
					...
					AllHandles.Add(NameData->PendingState.GetHandle());
					continue;
				}

				// Clear pending state
				constexpr const bool bCancelHandle = true;
				NameData->PendingState.Reset(bCancelHandle);
			}
			...
			for (const FName& BundleName : NewBundleState)
			{
				...
				{
					UE_LOGF(LogAssetManager, Verbose, "ChangeBundleStateForPrimaryAssets: No assets for bundle %ls::%ls", *PrimaryAssetId.ToString(), *BundleName.ToString());
				}
			}
			...
			if (PathsToLoad.Num() == 0)
			{
				...
				NameData->CurrentState.SetState({}, NewBundleState, LoadParams.DownloadParams);
				continue;
			}
			...
			TSharedPtr<FStreamableHandle> NewHandle = LoadAssetList(MoveTemp(PathsToLoad), MoveTemp(AssetListLoadParams), FString(DebugName), CopyTemp(Location));
			if (NewHandle && !NewHandle->HasLoadCompleted())
			{
				...
				NameData->PendingState.SetState(NewHandle, NewBundleState, LoadParams.DownloadParams);
			...
			{
				// OnAssetStateChangeCompleted requires this data to handle state transition correctly.
				NameData->PendingState.SetState({}, NewBundleState, LoadParams.DownloadParams);
...
void UAssetManager::OnAssetStateChangeCompleted(TSharedPtr<FStreamableHandle> BoundHandle, FPrimaryAssetId PrimaryAssetId)
{
	...
	if (NameData)
	{
		...
		if (!NameData->PendingState.GetHandle().IsValid() || NameData->PendingState.GetHandle() == BoundHandle)
		{
			// If an error occured, don't update the current state with a bad state
			if (BoundHandle->HasError())
			{
				const ::StreamableManager::FAdditionalContext* AdditionalContext = BoundHandle->GetError()->GetErrorContext<::StreamableManager::FAdditionalContext>();

				if (AdditionalContext)
				{
					UE_LOGF(LogAssetManager, Warning, "OnAssetStateChangeCompleted: Received error (%ls) loading asset! Primary asset id:(%ls) erroring asset:%ls", *BoundHandle->GetError()->GetErrorMessage(true).ToString(), *PrimaryAssetId.ToString(), *AdditionalContext->MissingObject);
				}
				else
				{
					UE_LOGF(LogAssetManager, Warning, "OnAssetStateChangeCompleted: Received error (%ls) loading asset! Primary asset id:(%ls)", *BoundHandle->GetError()->GetErrorMessage(true).ToString(), *PrimaryAssetId.ToString());
				}
			...
			{
				UE_LOGF(LogAssetManager, Verbose, "OnAssetStateChangeCompleted: Request was canceled (%ls)", *PrimaryAssetId.ToString());
			}
			else
			{
				NameData->CurrentState.MoveStateFrom(BoundHandle, MoveTemp(NameData->PendingState));
#if RHI_RESOURCE_PROVENANCE_ENABLED
				if (UMaterialParameterCollection* Collection = Cast<UMaterialParameterCollection>(
					GetPrimaryAssetObject(PrimaryAssetId)))
				{
					Collection->RecordAssetManagerProvenance(
						EMPCAssetProvenancePhase::AssetManagerLoadComplete,
						FString::Printf(
							TEXT("manager=%s primary_asset=%s handle=%p bundles=%d"),
							*GetClass()->GetPathName(),
							*PrimaryAssetId.ToString(),
							BoundHandle.Get(),
							NameData->CurrentState.GetBundleNames().Num()),
		...
		else if (NameData->PendingState.GetHandle().IsValid())
		{
			UE_LOGF(LogAssetManager, Verbose, "OnAssetStateChangeCompleted: Received after pending data changed, ignoring (%ls)", *PrimaryAssetId.ToString());
		}
	}
	else
	{
		UE_LOGF(LogAssetManager, Error, "OnAssetStateChangeCompleted: Received for invalid asset! (%ls)", *PrimaryAssetId.ToString());
	}
}
...
TSharedPtr<FStreamableHandle> UAssetManager::GetPrimaryAssetHandle(const FPrimaryAssetId& PrimaryAssetId, bool bForceCurrent, TArray<FName>* Bundles) const
{
	...
	if (Bundles)
	{
		*Bundles = LoadState.GetBundleNames();
	}
	return LoadState.GetHandle();
}
...
bool UAssetManager::GetPrimaryAssetsWithBundleState(TArray<FPrimaryAssetId>& PrimaryAssetList, const TArray<FPrimaryAssetType>& ValidTypes, const TArray<FName>& RequiredBundles, const TArray<FName>& ExcludedBundles, bool bForceCurrent) const
{
	...
	for (const TPair<FName, TSharedRef<FPrimaryAssetTypeData>>& TypePair : AssetTypeMap)
	{
		...
		for (const TPair<FName, FPrimaryAssetData>& NamePair : TypeData.GetAssets())
		{
			...
			{
				if (!LoadState.GetBundleNames().Contains(RequiredName))
				{
					bFailedTest = true;
					break;
				}
			...
			{
				if (LoadState.GetBundleNames().Contains(ExcludedName))
				{
					bFailedTest = true;
					break;
				}
...
void UAssetManager::GetPrimaryAssetBundleStateMap(TMap<FPrimaryAssetId, TArray<FName>>& BundleStateMap, bool bForceCurrent) const
{
	...
	for (const TPair<FName, TSharedRef<FPrimaryAssetTypeData>>& TypePair : AssetTypeMap)
	{
		...
		for (const TPair<FName, FPrimaryAssetData>& NamePair : TypeData.GetAssets())
		{
			...
			BundleStateMap.Add(AssetID, TArray<FName>(LoadState.GetBundleNames()));
		}
	}
}
	...
	for (const FPrimaryAssetId& PrimaryAssetId : AssetsToUnload)
	{
		...
		if (NameData)
		{
			...
			if (NameData->CurrentState.IsValid() || NameData->PendingState.IsValid())
			{
				...
				{
					Collection->RecordAssetManagerProvenance(
						EMPCAssetProvenancePhase::AssetManagerUnload,
						FString::Printf(
							TEXT("manager=%s primary_asset=%s current_handle=%p pending_handle=%p current_bundles=%d pending_bundles=%d"),
							*GetClass()->GetPathName(),
							*PrimaryAssetId.ToString(),
							NameData->CurrentState.GetHandle().Get(),
							NameData->PendingState.GetHandle().Get(),
							NameData->CurrentState.GetBundleNames().Num(),
							NameData->PendingState.GetBundleNames().Num()),
						UE::RHI::ResourceProvenance::CaptureCallerAddress());
				}
#endif
				NumUnloaded++;
...
TSharedPtr<FStreamableHandle> UAssetManager::LoadAssetList(TArray<FSoftObjectPath>&& AssetList, FAssetManagerLoadParams&& LoadParams, FString&& DebugName, UE::FSourceLocation&& Location)
{
	...
	if (bShouldAcquireMissingChunksOnLoad)
	{
		...
		if (ErrorChunks.Num() > 0)
		{
			...
			UE_LOGF(LogAssetManager, Error, "Failure loading %ls, Required chunk %d does not exist!", *DebugName, ErrorChunks[0]);
			return nullptr;
		}
	}
...
TSharedPtr<FStreamableHandle> UAssetManager::DownloadAssetList(TArray<FSoftObjectPath>&& AssetList, FAssetManagerLoadParams&& LoadParams, FString&& DebugName, UE::FSourceLocation&& Location)
{
	if (!LoadParams.DownloadParams)
	{
		UE_LOGF(LogAssetManager, Error, "Failure downloading %ls, no download params!", *DebugName);
		return nullptr;
	}
...
{
#if WITH_EDITOR
	UE_LOGF(LogAssetManager, Warning, "UAssetManager::OnAssetRegistryAvailableAfterInitialization is only supported in cooked builds, but was called from the editor!");
	return false;
#else
...

	{
		...
		
		{
			...
			bool bFileExists = PakPlatformFile != nullptr ? PakPlatformFile->FindFileInPakFiles(*Filename) : FPaths::FileExists(Filename);
			if (bFileExists && FFileHelper::LoadFileToArray(Bytes, *Filename))
			{
				bLoaded = true;
								...
								if (ShouldScanPrimaryAssetType(TypeData.Info))
								{
									...
									if (bFoundPath)
									{
										...
										if (AssetData.GetTagValue(GetEncryptionKeyAssetTagName(), GuidString))
										{
											...
											UE_LOGF(LogAssetManager, Verbose, "Found encrypted primary asset '%ls' using keys '%ls'", *PrimaryAssetId.PrimaryAssetName.ToString(), *GuidString);
										}
	...
	UE_CLOGF(bLoaded, LogAssetManager, Log, "Registered new asset registry '%ls' in %.4fs", *InName.ToString(), RegistrationTime);
	return bLoaded;
#endif
}
...
void UAssetManager::LoadRedirectorMaps()
{
	...
	for (const TPair<FSoftObjectPath, FSoftObjectPath>& Pair : AssetPathRedirects)
	{
		...
		while (FSoftObjectPath* NewPathValue = AssetPathRedirects.Find(NewPath)) // Does the NewPath exist as a key?
		{
			...
			{
				UE_LOGF(LogAssetManager, Error, "AssetPathRedirect cycle detected when redirecting: %ls to %ls", *OldPath.ToString(), *NewPath.ToString());
				break;
			}
			else
			{
...
void UAssetManager::DumpAssetTypeSummary()
{
	...
	UE_LOGF(LogAssetManager, Log, "=========== Asset Manager Type Summary ===========");

	for (const FPrimaryAssetTypeInfo& TypeInfo : TypeInfos)
	{
		UE_LOGF(LogAssetManager, Log, "  %ls: Class %ls, Count %d, Paths %ls", *TypeInfo.PrimaryAssetType.ToString(), *TypeInfo.AssetBaseClassLoaded->GetName(), TypeInfo.NumberOfAssets, *FString::Join(TypeInfo.AssetScanPaths, TEXT(", ")));
	}
}
...
void UAssetManager::DumpLoadedAssetState()
{
	...
	UE_LOGF(LogAssetManager, Log, "=========== Asset Manager Loaded Asset State ===========");
	...
	for (const FPrimaryAssetTypeInfo& TypeInfo : TypeInfos)
	{
		...
		for (const TPair<FName, FPrimaryAssetData>& NamePair : TypeData.GetAssets())
		{
			...
			if (NameData.PendingState.IsValid() || NameData.CurrentState.IsValid())
			{
				...
				for (const FName& BundleName : LoadState.GetBundleNames())
				{
		...
		{
			UE_LOGF(LogAssetManager, Log, "  Type %ls:", *TypeInfo.PrimaryAssetType.ToString());
			...
			{
				UE_LOGF(LogAssetManager, Log, "    %ls: %ls, (%ls)", *LoadedInfo.AssetName.ToString(), LoadedInfo.bPending ? TEXT("pending load") : TEXT("loaded"), *LoadedInfo.BundleState);
			}
		}	
	}
}
...
void UAssetManager::DumpBundlesForAsset(const TArray<FString>& Args)
{
	if (Args.Num() < 1)
	{
		UE_LOGF(LogAssetManager, Warning, "Too few arguments for DumpBundlesForAsset. Include the primary asset id (i.e. Map:Entry)");
		return;
	}
	...
	{
		UE_LOGF(LogAssetManager, Warning, "Incorrect argument for DumpBundlesForAsset. Arg should be the primary asset id (i.e. Map:Entry)");
		return;
	}
	...
	if (!FoundMap)
	{
		UE_LOGF(LogAssetManager, Display, "Could not find bundles for primary asset %ls.", *PrimaryAssetIdString);
		return;
	}

	UE_LOGF(LogAssetManager, Display, "Dumping bundles for primary asset %ls...", *PrimaryAssetIdString);
	for (const FAssetBundleEntry& Entry : (**FoundMap).Bundles)
	{
		UE_LOGF(LogAssetManager, Display, "  Bundle: %ls (%d assets)", *Entry.BundleName.ToString(), Entry.AssetPaths.Num());
		for (const FTopLevelAssetPath& Path : Entry.AssetPaths)
		{
			UE_LOGF(LogAssetManager, Display, "    %ls", *Path.ToString());
		}
	}
}
...
{
	UE_LOGF(LogAssetManager, Log, "=========== Asset Registry Summary ===========");
	UE_LOGF(LogAssetManager, Log, "Current Registry Memory:");
...
#if WITH_EDITOR
	UE_LOGF(LogAssetManager, Log, "Estimated Cooked Registry Memory:");
...
void UAssetManager::DumpReferencersForPackage(const TArray< FString >& PackageNames)
{
	...
	for (const FString& PackageString : PackageNames)
	{
		...
		for (const FAssetIdentifier& Identifier : FoundReferencers)
		{
			...
			UE_LOGF(LogAssetManager, Log, "%ls depends on %ls", *ReferenceString, *PackageString);
		}
	}
...
{
	static const FName NAME_EncryptionKey("EncryptionKey");
...
void UAssetManager::ScanPrimaryAssetTypesFromConfig()
{
	...
	for (FPrimaryAssetTypeInfo TypeInfo : Settings.PrimaryAssetTypesToScan)
	{
		...
		UE_CLOGF(AssetTypeMap.Find(TypeInfo.PrimaryAssetType), LogAssetManager, Error, "Found multiple \"%ls\" Primary Asset Type entries in \"Primary Asset Types To Scan\" config. Only a single entry per type is supported.", *TypeInfo.PrimaryAssetType.ToString());
...
void UAssetManager::ScanPrimaryAssetRulesFromConfig()
{
	...
	for (const FPrimaryAssetRulesOverride& Override : Settings.PrimaryAssetRules)
	{
		...
		{
			UE_LOGF(LogAssetManager, Error, "Cannot specify Rules overrides for Labels in ini! You most modify asset %ls!", *Override.PrimaryAssetId.ToString());
			continue;
		}
...
void UAssetManager::AddAssetSearchRoot(const FString& NewRootPath)
{
	...
	{
		UE_LOGF(LogAssetManager, Error, "AddAssetSearchRoot called twice with path %ls!", *NormalizedPath);
		return;
	}
...
bool UAssetManager::GetPackageCookRuleManagers(FName PackageName, bool bRecurseToParents,
	TMap<FPrimaryAssetId, UE::AssetRegistry::EDependencyProperty>& Managers) const
{
	return GetPackageManagersInternal(PackageName, true /* bUseCookRuleReferences */,
		bRecurseToParents, Managers);
}

bool UAssetManager::GetPackageManagers(FName PackageName, bool bRecurseToParents,
	TSet<FPrimaryAssetId>& ManagerSet) const
{
...
bool UAssetManager::GetPackageManagers(FName PackageName, bool bRecurseToParents,
	TMap<FPrimaryAssetId, UE::AssetRegistry::EDependencyProperty>& Managers) const
{
	return GetPackageManagersInternal(PackageName, false /* bUseCookRuleReferences */,
		bRecurseToParents, Managers);
}

bool UAssetManager::GetPackageManagersInternal(FName PackageName,
	bool bUseCookRuleReferences, bool bRecurseToParents,
	TMap<FPrimaryAssetId, UE::AssetRegistry::EDependencyProperty>& Managers) const
{
	IAssetRegistry& AssetRegistry = GetAssetRegistry();
	...
	// We have two ManagementParentMaps that are calculated using the two kinds of dependencies we track:
	// CookRule or Chunking. Pick the corresponding map based on bUseCookRuleReferencess.
	const TMap<FPrimaryAssetId, TArray<FPrimaryAssetId>>& UsedParentMap = bUseCookRuleReferences ?
		CookRuleManagementParentMap : ManagementParentMap;
	UE::AssetRegistry::EDependencyProperty RequiredProperties = bUseCookRuleReferences ?
		UE::AssetRegistry::EDependencyProperty::CookRule : UE::AssetRegistry::EDependencyProperty::None;

	bool bFoundAny = false;
	TArray<FAssetDependency> ReferencingPrimaryAssets;
	ReferencingPrimaryAssets.Reserve(128);

	UE::AssetRegistry::FDependencyQuery RequiredPropertiesQuery;
	RequiredPropertiesQuery.Required = RequiredProperties;
	AssetRegistry.GetReferencers(PackageName, ReferencingPrimaryAssets,
		UE::AssetRegistry::EDependencyCategory::Manage, RequiredPropertiesQuery);
	...
	for (int32 IdentifierIndex = 0; IdentifierIndex < ReferencingPrimaryAssets.Num(); IdentifierIndex++)
	{
		...
		bFoundAny = true;
		UE::AssetRegistry::EDependencyProperty& ExistingProperties = Managers.FindOrAdd(
			PrimaryAssetId, UE::AssetRegistry::EDependencyProperty::None);
		ExistingProperties = UnionManageProperties(ExistingProperties, AssetDependency.Properties);

		if (bRecurseToParents)
		{
			const TArray<FPrimaryAssetId>* ManagementParents = UsedParentMap.Find(PrimaryAssetId);
			if (ManagementParents)
			{
				// AssetDependency can be invalidated because we are modifying the array it is in.
				// Copy its data now before we modify the array.
...
void UAssetManager::WarnAboutInvalidPrimaryAsset(const FPrimaryAssetId& PrimaryAssetId, const FString& Message) const
{
	if (!WarningInvalidAssets.Contains(PrimaryAssetId))
	{
		...
		if (Settings.bShouldWarnAboutInvalidAssets)
		{
			...
			if (FoundType)
			{
				UE_LOGF(LogAssetManager, Warning, "Invalid Primary Asset Id %ls: %ls", *PrimaryAssetId.ToString(), *Message);
			}
			else
			{
				UE_LOGF(LogAssetManager, Warning, "Invalid Primary Asset Type %ls: %ls", *PrimaryAssetId.ToString(), *Message);
			}
		}
	}
}
...
void UAssetManager::UpdateManagementDatabase(EUpdateManagementDatabaseFlags InFlags)
{
	if (!GIsEditor)
	{
		...
		UE_LOGF(LogAssetManager, Error, "UpdateManagementDatabase does not work in standalone game because it doesn't load the entire Asset Registry!");
	}
	...
	CookRuleManagementParentMap.Reset();
	...
	for (const TPair<FName, TSharedRef<FPrimaryAssetTypeData>>& TypePair : AssetTypeMap)
	{
		...
		for (const TPair<FName, FPrimaryAssetData>& NamePair : TypeData.GetAssets())
		{
			...
			if (AssetRef.IsValid())
			{
				...
				{
					UE_LOGF(LogAssetManager, Warning, "Ignoring 'None' reference originating from %ls from NameData", *PrimaryAssetId.ToString());
				}
				else
				{
			...
			if (const TSharedPtr<FAssetBundleData, ESPMode::ThreadSafe>* BundleMap = CachedAssetBundles.Find(PrimaryAssetId))
			{
				for (const FAssetBundleEntry& Entry : (**BundleMap).Bundles)
				{
					for (const FTopLevelAssetPath& BundleAssetRef : Entry.AssetPaths)
					{
						...
						{
							UE_LOGF(LogAssetManager, Warning, "Ignoring 'None' reference originating from %ls from Bundle %ls", *PrimaryAssetId.ToString(), *PrimaryAssetId.ToString());
						}
						else
						{
							AssetPackagesReferenced.Add(PackageName);
	...
	for (const TPair<FName, TSharedRef<FPrimaryAssetTypeData>>& TypePair : AssetTypeMap)
	{
		...
		for (const TPair<FName, FPrimaryAssetData>& NamePair : TypeData.GetAssets())
		{
			...
			// ChunkManagers are a superset of CookRuleManagers; we rely on that to avoid the need
			// for querying CookRuleManagers when Managers is empty.
			TMap<FPrimaryAssetId, UE::AssetRegistry::EDependencyProperty> Managers;
			...
			if (AssetRef.IsValid())
			{
				...
				if (GetPackageManagers(PackageName, false, Managers) && Managers.Num() > 1)
				{
					...
					for (const TPair<FPrimaryAssetId, UE::AssetRegistry::EDependencyProperty>& ManagerPair : Managers)
					{
						if (ManagerPair.Key != PrimaryAssetId)
						{
							// Update the cached version and the version in registry
							if (EnumHasAnyFlags(ManagerPair.Value, UE::AssetRegistry::EDependencyProperty::CookRule))
							{
								CookRuleManagementParentMap.FindOrAdd(PrimaryAssetId).AddUnique(ManagerPair.Key);
							}
							ManagementParentMap.FindOrAdd(PrimaryAssetId).AddUnique(ManagerPair.Key);

							PrimaryAssetIdManagementMap.Add(ManagerPair.Key, PrimaryAssetId);
						}
					}
				}
			}
			...
			{
				Managers.Add(PrimaryAssetId, UE::AssetRegistry::EDependencyProperty::ManageMask);
			}

			// Compute chunk assignments for the PrimaryAsset and record them as manage references from a
			// PrimaryAssetId we create for each chunk
			ChunkList.Reset();
			TSet<FPrimaryAssetId> ManagersSet;
			Managers.GetKeys(ManagersSet);
			GetPrimaryAssetSetChunkIds(ManagersSet, nullptr, ExistingChunkList, ChunkList);
...
void UAssetManager::ModifyCook(TConstArrayView<const ITargetPlatform*> TargetPlatforms, TArray<FName>& PackagesToCook, TArray<FName>& PackagesToNeverCook)
{
	...
	for (const ITargetPlatform* TargetPlatform : TargetPlatforms.Slice(1,TargetPlatforms.Num() - 1))
	{
		if (TargetPlatform->AllowsDevelopmentObjects() != bTargetPlatformsAllowDevelopmentObjects)
		{
			...
			UE_LOGF(LogAssetManager, Error,
				"Cooking platform %ls and %ls in a single cook is not supported, because they have different values for AllowsDevelopmentObjects. " "This cook session will use AllowsDevelopmentObjects = true, which will add packages to platform %ls that should not be present.",
...
{
	UE_LOGF(LogAssetManager, Display, "ModifyDLCCook: Scanning Plugin Directory %ls for assets, and adding them to the cook list", *DLCName);
	FString DLCPath;
	...
	IFileManager::Get().FindFilesRecursive(Files, *DLCPath, *(FString("*") + FPackageName::GetAssetPackageExtension()), true, false, false);
	IFileManager::Get().FindFilesRecursive(Files, *DLCPath, *(FString("*") + FPackageName::GetMapPackageExtension()), true, false, false);
...
void UAssetManager::GatherPublicAssetsForPackage(FName PackagePath, TSet<FSoftObjectPath>& AssetsToCook) const
{
	...
	{
		// Only TopLevelAssets can be API exposed
		if (AssetData.IsTopLevelAsset())
		{
			// this package can be externally referenced; include it in the cook
			if (!AssetData.PackageName.IsNone())
			{
				UE_LOGF(LogAssetManager, Verbose,
					"GatherPublicAssetsForPackage: Adding public package [%ls] (instigator: [%ls]",
			...
			{
				UE_LOGF(LogAssetManager, Error,
					"GatherPublicAssetsForPackage: Failed to resolve package for asset [%ls]",
					*AssetData.AssetName.ToString());
			}
		}
...
EPrimaryAssetCookRule UAssetManager::GetPackageCookRule(FName PackageName) const
{
	...
	TMap<FPrimaryAssetId, UE::AssetRegistry::EDependencyProperty> Managers;
	GetPackageCookRuleManagers(PackageName, true, Managers);
	...
	if (ConflictIds)
	{
		UE_LOGF(LogAssetManager, Error, "GetPackageCookRule: Conflicting Cook Rule for package %ls! %ls and %ls have the same priority and disagree.",
...
bool UAssetManager::VerifyCanCookPackage(UE::Cook::ICookInfo* CookInfo, FName PackageName, bool bLogError) const
{
	...
	if (CookRule == EPrimaryAssetCookRule::NeverCook)
	{
		if (bLogError)
		{
			UE_LOGF(LogAssetManager, Error, "Package %ls is set to NeverCook, but something is trying to cook it! Instigators: %ls",
		...
		&& bOnlyCookProductionAssets && !bTargetPlatformsAllowDevelopmentObjects)
	{
		if (bLogError)
		{
			UE_LOGF(LogAssetManager, Warning, "Package %ls is set to ProductionNeverCook, and bOnlyCookProductionAssets is true, but something is trying to cook it! Instigators: %ls",
				*PackageName.ToString(), *GetInstigatorChainString(CookInfo, PackageName));
		}
	...
	if (!bRetVal && bLogError)
	{
		TMap<FPrimaryAssetId, UE::AssetRegistry::EDependencyProperty> Managers;
		GetPackageCookRuleManagers(PackageName, true, Managers);
		UE_LOGF(LogAssetManager, Display, "Listing Managers... (Count:%d)", Managers.Num());
		for (const TPair<FPrimaryAssetId, UE::AssetRegistry::EDependencyProperty>& PrimaryAssetPair : Managers)
		{
			UE_LOGF(LogAssetManager, Display, "  %ls", *PrimaryAssetPair.Key.ToString());
		}
		...
		UE_LOGF(LogAssetManager, Display, "Listing known direct referencers... (Count:%d)", PackageReferencers.Num());
		for (FName PackageReferencer : PackageReferencers)
		{
			UE_LOGF(LogAssetManager, Display, "  %ls", *PackageReferencer.ToString());
		}
	}
...
bool UAssetManager::GetPackageChunkIds(FName PackageName, const ITargetPlatform* TargetPlatform, TArrayView<const int32> ExistingChunkList, TArray<int32>& OutChunkList, TArray<int32>* OutOverrideChunkList) const
{
	...
	TSet<FPrimaryAssetId> ManagersSet;
	Managers.GetKeys(ManagersSet);
	return GetPrimaryAssetSetChunkIds(ManagersSet, TargetPlatform, ExistingChunkList, OutChunkList);
}
...
bool UAssetManager::GetPrimaryAssetSetChunkIds(const TSet<FPrimaryAssetId>& PrimaryAssetSet, const class ITargetPlatform* TargetPlatform,
	TArrayView<const int32> ExistingChunkList, TArray<int32>& OutChunkList) const
{
	return GetPrimaryAssetSetChunkIds(PrimaryAssetSet.Array(), TargetPlatform, ExistingChunkList, OutChunkList);
}

bool UAssetManager::GetPrimaryAssetSetChunkIds(TConstArrayView<FPrimaryAssetId> PrimaryAssets, const class ITargetPlatform* TargetPlatform,
	TArrayView<const int32> ExistingChunkList, TArray<int32>& OutChunkList) const
{
	bool bFoundAny = false;
	int32 HighestChunk = 0;
	for (const FPrimaryAssetId& PrimaryAssetId : PrimaryAssets)
	{
		FPrimaryAssetRules Rules = GetPrimaryAssetRules(PrimaryAssetId);
...
TMap<int32, FAssetManagerChunkInfo> UAssetManager::BuildChunkMap(const TSet<FName>& PackagesToUpdateChunksFor, const TSet<FName>& StartupPackages, const TSet<FName>& PackageThatWereCooked) const
{
	...
	for (const TPair<FName, TSharedRef<FPrimaryAssetTypeData>>& TypePair : AssetTypeMap)
	{
		...
		for (const TPair<FName, FPrimaryAssetData>& NamePair : TypeData.GetAssets())
		{
			...
			{
				FName PackageName = FName(*AssetRef.GetLongPackageName());
				TMap<FPrimaryAssetId, UE::AssetRegistry::EDependencyProperty> ManagersMap;
				GetPackageManagers(PackageName, false, ManagersMap);
				ManagersMap.GetKeys(Managers);
			}
			else
			{
...
void UAssetManager::OnObjectPreSave(UObject* Object, FObjectPreSaveContext SaveContext)
{
	...
	if (FoundPrimaryAssetId.IsValid())
	{
		...
		{
			UE_LOGF(LogAssetManager, Error, "Registered PrimaryAssetId %ls for asset %ls does not match object's real id of %ls! This will not load properly at runtime!", *FoundPrimaryAssetId.ToString(), *Object->GetPathName(), *ObjectPrimaryAssetId.ToString());
		}
	}
}
...
{
	static FName AssetBundlesName = "AssetBundles";
	static FName IncludeAssetBundlesName = "IncludeAssetBundles";
	...
	for (TPropertyValueIterator<const FProperty> It(Struct, StructValue); It; ++It)
	{
		...
		if (!FoundRef.IsNull())
		{
			...
			{
				UE_LOGF(LogAssetManager, Error, "Asset bundle reference with invalid package name in %ls. Property:%ls", *DebugName.ToString(), *GetNameSafe(Property));
			}
		}
	}
}
		...
		[](const TArray<FString>& Params, UWorld* World)
{
	if (Params.Num() == 0)
	{
		UE_LOGF(LogAssetManager, Log, "No types specified");
	}
	...
	for (const FString& Param : Params)
	{
		...
		{
			UE_LOGF(LogAssetManager, Log, "LoadPrimaryAssetsWithType(%ls)", *Param);
			UAssetManager::Get().LoadPrimaryAssetsWithType(TypeToLoad);
		}
		else
		{
			UE_LOGF(LogAssetManager, Log, "Cannot get type info for PrimaryAssetType %ls", *Param);
		}		
	}
}), ECVF_Cheat);
		...
		[](const TArray<FString>& Params, UWorld* World)
{
	...
	for (const FString& Param : Params)
	{
		...
		if (UAssetManager::Get().GetPrimaryAssetTypeInfo(TypeToUnload, /*out*/ Info))
		{
			...
			UE_LOGF(LogAssetManager, Log, "UnloadPrimaryAssetsWithType(%ls): Unloaded %d assets", *Param, NumUnloaded);
		}
		else
		{
			UE_LOGF(LogAssetManager, Log, "Cannot get type info for PrimaryAssetType %ls", *Param);
