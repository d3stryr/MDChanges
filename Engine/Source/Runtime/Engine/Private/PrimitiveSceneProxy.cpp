...
namespace
{
	struct FPrimitiveContributorDataLayer
	{
		const UDataLayerInstance* Instance = nullptr;
		bool bActorMember = false;
		bool bRuntimeCellMember = false;
	};

	const TCHAR* GetContributorDataLayerStateName(EDataLayerRuntimeState State)
	{
		switch (State)
		{
	...
	void AddContributorDataLayer(
		TArray<FPrimitiveContributorDataLayer>& DataLayers,
		const UDataLayerInstance* Instance,
		bool bActorMember,
		bool bRuntimeCellMember)
	{
		if (!Instance)
		{
			return;
		...
		FPrimitiveContributorDataLayer* Existing = DataLayers.FindByPredicate(
			[Instance](const FPrimitiveContributorDataLayer& Entry)
			{
				return Entry.Instance == Instance;
			});
		if (Existing)
		{
			Existing->bActorMember |= bActorMember;
			Existing->bRuntimeCellMember |= bRuntimeCellMember;
			return;
		}

		FPrimitiveContributorDataLayer& Entry = DataLayers.AddDefaulted_GetRef();
		Entry.Instance = Instance;
		Entry.bActorMember = bActorMember;
		Entry.bRuntimeCellMember = bRuntimeCellMember;
	}

	void RecordPrimitiveContributorMetadata(
		const FPrimitiveSceneProxyDesc& ProxyDesc,
		const FPrimitiveSceneProxy* Proxy,
		uint64 ContributorId)
	{
		using namespace UE::RHI::ResourceProvenance;
		if (ContributorId == 0)
		{
			return;
		}
		...
		if (!IsInGameThread())
		{
			RecordContributorMetadata(
				EOperation::ContributorRegistered,
				ContributorId,
				Proxy,
				0,
				TEXT("kind=primitive_scene_proxy metadata_omitted=not_game_thread"),
		...
		RecordContributorMetadata(
			...
			Proxy,
			PackedRegistration,
			TEXT("kind=primitive_scene_proxy"),
			CaptureCallerAddress());
		TArray<FPrimitiveContributorDataLayer> DataLayers;
		TArray<const UDataLayerInstance*> ActorDataLayerInstances;
		TArray<const UDataLayerInstance*> CellDataLayerInstances;
		if (OwnerActor)
		{
			ActorDataLayerInstances = OwnerActor->GetDataLayerInstances();
			for (const UDataLayerInstance* Instance : ActorDataLayerInstances)
			{
				AddContributorDataLayer(DataLayers, Instance, true, false);
			}
		}
		if (RuntimeCell)
		{
			CellDataLayerInstances = RuntimeCell->GetDataLayerInstances();
			for (const UDataLayerInstance* Instance : CellDataLayerInstances)
			{
				AddContributorDataLayer(DataLayers, Instance, false, true);
			}
		}
		...
		if (ProxyDesc.Component)
		{
			...
			ComponentText.Appendf(
				...
				*Proxy->GetLevelName().ToString(),
				*ProxyDesc.GetPathName());
			RecordContributorMetadata(
				EOperation::ContributorComponent,
				ContributorId,
				ProxyDesc.Component,
				0,
		...
		if (RuntimeCell)
		{
			...
			CellText.Appendf(
				...
				*RuntimeCell->GetExternalDataLayer().ToString(),
				static_cast<uint32>(CellDataLayerInstances.Num()),
				*RuntimeCell->GetDebugName());
			RecordContributorMetadata(
				EOperation::ContributorWorldPartition,
				ContributorId,
		...
		const uint32 TotalDataLayers = static_cast<uint32>(DataLayers.Num());
		const uint32 CapturedDataLayers = FMath::Min(TotalDataLayers, GetMaxContributorDataLayers());
		for (uint32 DataLayerIndex = 0; DataLayerIndex < CapturedDataLayers; ++DataLayerIndex)
		{
			const FPrimitiveContributorDataLayer& Entry = DataLayers[DataLayerIndex];
			const bool bRuntime = Entry.Instance->IsRuntime();
			const EDataLayerRuntimeState RuntimeState = bRuntime
				? Entry.Instance->GetRuntimeState()
			...
			DataLayerText.Appendf(
				...
				*Entry.Instance->GetDataLayerFullName(),
				...
				GetContributorDataLayerStateName(RuntimeState),
				GetContributorDataLayerStateName(EffectiveRuntimeState),
				static_cast<uint32>(Entry.bActorMember),
				static_cast<uint32>(Entry.bRuntimeCellMember),
				*Entry.Instance->GetPathName());
			const uint32 PackedDataLayer =
				(FMath::Min<uint32>(TotalDataLayers, MAX_uint16) << 16) |
				FMath::Min<uint32>(DataLayerIndex, MAX_uint16);
			RecordContributorMetadata(
				EOperation::ContributorDataLayer,
				ContributorId,
				Entry.Instance,
				PackedDataLayer,
		...
		if (CapturedDataLayers < TotalDataLayers)
		{
			const uint32 OmittedDataLayers = TotalDataLayers - CapturedDataLayers;
			RecordContributorMetadata(
				EOperation::ContributorCoverageOmitted,
				ContributorId,
				Proxy,
				OmittedDataLayers,
				TEXT("reason=max_data_layers_per_contributor"),
				CaptureCallerAddress());
		}
	}
}
...
#if RHI_RESOURCE_PROVENANCE_ENABLED
	ProvenanceContributorId = UE::RHI::ResourceProvenance::AllocateContributorId();
	RecordPrimitiveContributorMetadata(InProxyDesc, this, ProvenanceContributorId);
#endif
...
{
#if RHI_RESOURCE_PROVENANCE_ENABLED
	UE::RHI::ResourceProvenance::RecordContributorMetadata(
		UE::RHI::ResourceProvenance::EOperation::ContributorRetired,
		ProvenanceContributorId,
		this,
		0,
		TEXT("reason=primitive_scene_proxy_destroyed"),
