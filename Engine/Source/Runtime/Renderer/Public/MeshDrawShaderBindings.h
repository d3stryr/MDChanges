...
#if RHI_RESOURCE_PROVENANCE_ENABLED
			UE::RHI::ResourceProvenance::RecordBindingStore(
				Value,
				...
				if (LooseParameterBuffer.BaseIndex == Parameter.GetBufferIndex())
				{
					...
					for (int32 LooseParameterIndex = 0; LooseParameterIndex < Parameters.Num(); LooseParameterIndex++)
					{
						...
						if (Parameter.GetBaseIndex() == LooseParameter.BaseIndex)
						{
							...
							ensureMsgf(sizeof(ParameterType) == Parameter.GetNumBytes(), TEXT("Size mismatch between the value being set and the size the shader requires.  Setting %" SIZE_T_FMT " bytes on loose parameter at BaseIndex %u, which requires %u bytes.  This can cause GPU hangs, depending on usage."), sizeof(ParameterType), Parameter.GetBaseIndex(), Parameter.GetNumBytes());
...
#if RHI_RESOURCE_PROVENANCE_ENABLED
	inline uint64* GetProvenanceOwnerStart() const
	{
...
#if RHI_RESOURCE_PROVENANCE_ENABLED
	inline void WriteProvenanceOwnerKey(uint64 OwnerKey, uint32 BaseIndex)
	{
		const int32 FoundIndex = FindSortedArrayBaseIndex(MakeArrayView(ParameterMapInfo.UniformBuffers), BaseIndex);
		if (FoundIndex >= 0)
		{
			GetProvenanceOwnerStart()[FoundIndex] = OwnerKey;
		}
