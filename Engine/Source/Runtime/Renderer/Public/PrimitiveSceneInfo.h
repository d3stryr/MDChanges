...
struct FPrimitiveFlagsCompact
{
	...
	/** True if the proxy is force hidden. */
	uint8 bIsForceHidden : 1;
...
public:
	...
	UE_DEPRECATED(5.8, "CachedReflectionCaptureProxies is no longer initialized, and will be removed in a future release.  Use CachedReflectionCaptureProxy.")
	...
	void RemoveFromScene(bool bUpdateStaticDrawLists, uint64 ProvenanceTeardownId = 0);
	...
	void RemoveStaticMeshes(uint64 ProvenanceTeardownId = 0);
	...
	/** Cache per-primitive reflection captures used for mobile/forward rendering.  Returns true if the cached reflection captures changed. */
	bool CacheReflectionCaptures();

	/** Nulls out the cached per-primitive reflection capture, if it matches ChangedProxy or ChangedProxy is null. */
	void RemoveCachedReflectionCapture(const FReflectionCaptureProxy* ChangedProxy = nullptr);

	/** Nulls out the cached per-primitive planar reflection, if it matches ChangedProxy or ChangedProxy is null. */
	void RemoveCachedPlanarReflection(const FPlanarReflectionSceneProxy* ChangedProxy = nullptr);
	...
	bool IsCachedRayTracingGeometryValid(int8 LODIndex, int32 InstanceIndex) const;

	UE_DEPRECATED(5.8, "Please specify both LODIndex and InstanceIndex.")
	bool IsCachedRayTracingGeometryValid(int8 LODIndex) const
	{
		return IsCachedRayTracingGeometryValid(LODIndex, 0);
	}

	UE_DEPRECATED(5.8, "Use GetRayTracingLODData() instead.")
	RENDERER_API const FRayTracingGeometry* GetStaticRayTracingGeometry(int8 LODIndex) const;
	UE_DEPRECATED(5.8, "Use GetRayTracingLODData() instead.")
	RENDERER_API const FRayTracingGeometry* GetValidStaticRayTracingGeometry(int8& InOutLODIndex) const;

	int32 GetStaticRayTracingGeometryNum() const { return RayTracingLODData.Num(); }

	UE_DEPRECATED(5.7, "Use GetRayTracingLODData() instead.")
	const FRayTracingGeometry* GetCachedRayTracingGeometry() const { return nullptr; }
#endif
...
public:
	...
	/** Whether the primitive is newly registered or moved and CachedReflectionCaptureProxy needs to be updated on the next render. Not a bitfield to avoid parallel access with other bits. */
	bool bNeedsCachedReflectionCaptureUpdate = true;

#if RHI_RAYTRACING
	bool : 0; // force separate storage unit to avoid race conditions with tasks modifying other non-raytracing bits.
	...
	bool : 0; // force separate storage unit to avoid race condition modifying bIsCachedRayTracingInstanceValid and bDynamicRayTracingInstanceCachedDataDirty.
	...
	struct FRayTracingInstanceData
	{
	...
	struct FRayTracingLODData
	{
		...
		TArray<FRayTracingInstanceData, TInlineAllocator<1>> Instances;
	};

	// TODO: rename to GetStaticRayTracingLODNum ?
	...
	UE_DEPRECATED(5.8, "Get index for specific instances via FRayTracingLODData instead.")
	const uint32 GetMainRayTracingInstanceIndex() const
	{
		if (RayTracingLODData.IsEmpty() || RayTracingLODData[0].Instances.IsEmpty())
		{
			return UINT32_MAX;
		}

		return RayTracingLODData[0].Instances[0].RayTracingInstanceIndexMain;
	}
	...
	const uint32 GetDecalRayTracingInstanceIndex() const
	{
		...
		return RayTracingLODData[0].Instances[0].RayTracingInstanceIndexDecal;
	}

	// TODO: rename to SetStaticRayTracingLODData ?
...
private:
	...
	void AllocateRayTracingSBT(FRayTracingInstanceData& InstanceData, const FRHIRayTracingGeometry* CachedRayTracingInstanceGeometryRHI, uint32 CachedRayTracingInstanceSegmentCount);

	void CacheRayTracingShaderBindingData(FRayTracingInstanceData& InstanceData, const FRHIRayTracingGeometry* CachedRayTracingInstanceGeometryRHI);
	void CacheRayTracingInstance(FRayTracingInstanceData& InstanceData, FRayTracingGeometryInstance Instance);
...
private:
	...
	void RemoveCachedMeshDrawCommands(uint64 ProvenanceTeardownId = 0);
