...
inline const TCHAR* GetMeshPassName(EMeshPass::Type MeshPass)
{
	...
	case EMeshPass::FrontLayerTranslucencyGBuffer: return TEXT("FrontLayerTranslucencyGBuffer");
...
struct FMinimalBoundShaderStateInput
{
	...
	FMinimalBoundShaderStateInput(const FMinimalBoundShaderStateInput & Other)
		: VertexDeclarationRHI(Other.VertexDeclarationRHI)
		, CachedVertexShader(Other.CachedVertexShader.load(std::memory_order_relaxed))
		, CachedPixelShader(Other.CachedPixelShader)
		, bAllShaderAreLoaded(Other.bAllShaderAreLoaded)
#if PLATFORM_SUPPORTS_GEOMETRY_SHADERS
		, CachedGeometryShader(Other.CachedGeometryShader)
#endif
#if PLATFORM_SUPPORTS_MESH_SHADERS
		, CachedMeshShader(Other.CachedMeshShader)
#endif
		, VertexShaderResource(Other.VertexShaderResource)
		, PixelShaderResource(Other.PixelShaderResource)
#if PLATFORM_SUPPORTS_GEOMETRY_SHADERS
		, GeometryShaderResource(Other.GeometryShaderResource)
#endif
#if PLATFORM_SUPPORTS_MESH_SHADERS
		, MeshShaderResource(Other.MeshShaderResource)
#endif
		, VertexShaderIndex(Other.VertexShaderIndex)
		, PixelShaderIndex(Other.PixelShaderIndex)
#if PLATFORM_SUPPORTS_GEOMETRY_SHADERS
		, GeometryShaderIndex(Other.GeometryShaderIndex)
#endif
#if PLATFORM_SUPPORTS_MESH_SHADERS
		, MeshShaderIndex(Other.MeshShaderIndex)
#endif
	{}

	FBoundShaderStateInput AsBoundShaderState(bool bRequired = true) const
	{
		bool bLocalAllShaderAreLoaded = true;
		bool bCanSkipShader = AllowSkipUnloadedShaders();
		const bool bShaderRequired = bRequired && !bCanSkipShader;
		...
		FRHIVertexShader* LocalCachedVertexShader = CachedVertexShader.load(std::memory_order_acquire);
		if (!LocalCachedVertexShader || !bAllShaderAreLoaded)
		{
			CachedPixelShader = PixelShaderResource ? static_cast<FRHIPixelShader*>(GetShaderResource(PixelShaderResource->GetShader(PixelShaderIndex, bShaderRequired))) : nullptr;
#if PLATFORM_SUPPORTS_GEOMETRY_SHADERS
			CachedGeometryShader = GeometryShaderResource ? static_cast<FRHIGeometryShader*>(GetShaderResource(GeometryShaderResource->GetShader(GeometryShaderIndex, bShaderRequired))) : nullptr;
#endif
#if PLATFORM_SUPPORTS_MESH_SHADERS
			CachedMeshShader = MeshShaderResource ? static_cast<FRHIMeshShader*>(GetShaderResource(MeshShaderResource->GetShader(MeshShaderIndex, bShaderRequired))) : nullptr;
#endif
			LocalCachedVertexShader = VertexShaderResource ? static_cast<FRHIVertexShader*>(GetShaderResource(VertexShaderResource->GetShader(VertexShaderIndex, bShaderRequired))) : nullptr;

			bAllShaderAreLoaded = bLocalAllShaderAreLoaded;
			CachedVertexShader.store(LocalCachedVertexShader, std::memory_order_release);
		}
...
#endif
			LocalCachedVertexShader = VertexShaderResource ? static_cast<FRHIVertexShader*>(VertexShaderResource->GetShader(VertexShaderIndex)) : nullptr;
	...
	mutable std::atomic<FRHIVertexShader*> CachedVertexShader = nullptr;
...
public:
	...
	FGraphicsPipelineStateInitializer AsGraphicsPipelineStateInitializer(bool bRequired = true) const
	{
		FGraphicsPipelineStateInitializer result(
			BoundShaderState.AsBoundShaderState(bRequired)
			, BlendState
			, RasterizerState
			, DepthStencilState
			, PrimitiveType
...
public:
	...
	virtual ~FMeshPassDrawListContext() = default;
