	...
	FShaderBindingState& RESTRICT ShaderBindingState)
{
	...
	const uint64* RESTRICT ProvenanceOwnerKeys = SingleShaderBindings.GetProvenanceOwnerStart();
#endif
	...
	for (int32 UniformBufferIndex = 0; UniformBufferIndex < NumUniformBuffers; UniformBufferIndex++)
	{
		...
		{
#if RHI_RESOURCE_PROVENANCE_ENABLED
			UE::RHI::ResourceProvenance::StageAccessOwner(
				UniformBuffer,
				ProvenanceOwnerKeys[UniformBufferIndex]);
#endif
...
{
	// The following members are potentially invalid when using bucketing (since multiple proxies can share the same MDC, even in the case of materials)
	DebugData.PrimitiveSceneProxyIfNotUsingStateBuckets = PrimitiveSceneProxy;
	DebugData.MaterialRenderProxyIfNotUsingStateBuckets = MaterialRenderProxy;
	...
	const FMeshBatchElement& BatchElement) const
{
	...
	if (UseGPUScene(GShaderPlatformForFeatureLevel[FeatureLevel]))
	{
...
FCachedPassMeshDrawListContext::FCachedPassMeshDrawListContext(FScene& InScene)
	: Scene(InScene)
	, bUseGPUScene(UseGPUScene(GMaxRHIShaderPlatform))
{
}
