	...
	const FPrimitiveSceneProxy* PrimitiveSceneProxy)
{
	...
	const uint64 OwnerKey = MaterialKey ^
		(SourceKey + 0x9e3779b97f4a7c15ull + (MaterialKey << 6) + (MaterialKey >> 2));
	bool bNeedsOwnerText = false;
	const uint64 ResourceId = UE::RHI::ResourceProvenance::ClaimAccessOwner(
		UniformBuffer,
		OwnerKey,
		bNeedsOwnerText);
	if (ResourceId == 0)
...
void FMaterialShader::GetShaderBindings(
	...
	const FPrimitiveSceneProxy* ProvenancePrimitiveSceneProxy) const
{
...

	{
		...
		{			
			...
			const uint64 AccessOwnerKey = RecordMaterialParameterCollectionAccessOwner(
				...
				0x4341434845444d45ull,
				MaterialRenderProxy,
				Material,
				ProvenancePrimitiveSceneProxy);
...
{
	FMaterialShader::GetShaderBindings(Scene, FeatureLevel, MaterialRenderProxy, Material, ShaderBindings, PrimitiveSceneProxy);
	...
	FVertexInputStreamArray& VertexStreams) const
{
	...
	if (UseGPUScene(ShaderPlatform) && VertexFactory->GetPrimitiveIdStreamIndex(FeatureLevel, InputStreamType) >= 0 && !PlatformUsesPrimitiveUB)
	{
