	...
	for (int32 CommandIndex = 0; CommandIndex < StaticMeshCommandInfos.Num(); ++CommandIndex)
	{
		...
		if (CachedCommand.StateBucketId != INDEX_NONE)
		{
...

			{
				...
				{
#if RHI_RESOURCE_PROVENANCE_ENABLED
					ElementKVP.Key.ShaderBindings.RecordProvenanceInvalidation(ProvenanceInvalidationCaller);
#endif
...
#if RHI_RESOURCE_PROVENANCE_ENABLED
			PassDrawList.MeshDrawCommands[CachedCommand.CommandIndex].ShaderBindings.RecordProvenanceInvalidation(ProvenanceInvalidationCaller);
#endif
