...
public:
	inline uint32 AddRef() const
	{
		...
		int32 NewValue = AtomicFlags.AddRef(this, ResourceId, ResourceTypeValue, CallerAddress, std::memory_order_acquire);
#else
	...
	inline uint32 Release() const
	{
		...
		const uint8 ResourceTypeValue = static_cast<uint8>(ResourceType);
		const uint64 CallerAddress = UE::RHI::ResourceProvenance::CaptureCallerAddress();
		int32 NewValue = AtomicFlags.Release(this, ResourceId, ResourceTypeValue, CallerAddress, std::memory_order_release);
#else
