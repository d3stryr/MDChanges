...
- typed release causes snapshot every still-live retained cached binding, including binding ID, owner key, live-copy count, invalidated/submitted flags, last operation, and explicit coverage;
- primitive contributors record actor/component paths, runtime grid, spatial-loading policy, World Partition cell GUID/package/external layer, and actor/cell Data Layer memberships and states before crossing to the render thread;
- a third cached uniform-buffer sidecar carries the immutable contributor ID, and `BindingContributor` rows link that contributor to the exact binding and MPC resource generation;
- Data Layer state requests, rejected/no-op outcomes, target-state changes, and effective-state changes carry one transition ID; descendant effective-state changes caused by a parent transition share that ID;
...
python Engine/Build/BatchFiles/DecodeRHIResourceProvenance.py <journal.rhiprov> --binding 910
python Engine/Build/BatchFiles/DecodeRHIResourceProvenance.py <journal.rhiprov> --contributor 314
python Engine/Build/BatchFiles/DecodeRHIResourceProvenance.py <journal.rhiprov> --data-layer-transition 27
```
...
- 16 retained live-binding states per priority resource generation; omitted states are reported by release snapshots and assertion coverage
...
- 16 Data Layer rows per contributor by default, configurable from 1–256 with `r.RHI.ResourceProvenance.MaxDataLayersPerContributor`
- 65,536 Data Layer transitions per process by default, configurable from 1–1,048,576 with `r.RHI.ResourceProvenance.MaxDataLayerTransitions`; first and aggregate omissions are reported
...
- Record `CellStateRequest`, `CellStateAccepted`, `CellStateBlocked`, `CellStateProgress`, `CellStateCompleted`, and `CellTransitionSuperseded` for World Partition runtime cells.
- Instrument policy-side load, activate, deactivate, and unload decisions, including explicit streaming-disabled, budget-exhausted, and cannot-unload blocks.
- Carry one bounded transition ID and requested terminal state on the runtime cell until `OnCellShown`, `OnCellHidden`, or the dynamic level-streaming callback confirms completion or failure.
- Pack prior/requested/observed cell state, operation, always/spatial/Data Layer flags, level-streaming state, and blocked reason into a fixed `uint32`.
- Copy cell GUID, debug name, object path, level package, external Data Layer, and Data Layer names only when the request begins on the game thread. The opaque runtime-cell address is the same join key used by Feature 6 `ContributorWorldPartition` rows.
...
- Record `DataLayerStateRequest`, `DataLayerStateRejected`, `DataLayerTargetStateChanged`, `DataLayerEffectiveStateChanged`, and `DataLayerStateNoOp` rows for World Partition runtime Data Layers.
- Use one bounded transition ID to link the request and outcome. Effective-state changes propagated to child Data Layers while resolving the request reuse the same transition ID.
- Pack old/new target state, old/new effective state, recursive flag, client/server-only flags, net mode, and rejection reason into a fixed `uint32`.
- Copy the world, parent, full Data Layer name, and Data Layer object path only on the game thread. The opaque `UDataLayerInstance` address matches Feature 6 `ContributorDataLayer` rows offline without dereferencing it from RHI/render threads.
- Capture effective-state recomputes that occur outside an explicit target-state request under their own transition ID.
- Bound capture to 65,536 transitions by default with `r.RHI.ResourceProvenance.MaxDataLayerTransitions`; the first omission is journaled and aggregate omissions appear in assertion coverage.
- Add TSV `data_layer_transition_id`, human-readable packed-state decoding, and `--data-layer-transition <id>` filtering.
- Python syntax, a synthetic transition decode/filter, delimiter/preprocessor balance, operation/declaration wiring, and source-blob checks passed. No PS5 SDK compile has been run.

### 2026-09-25 — Feature 6: World Partition and Data Layer contributor attribution

- Allocate a bounded immutable contributor ID when a primitive scene proxy is constructed and record metadata while actor/component UObjects are valid on the game thread.
- Capture actor path, runtime grid, spatial-loading flag, component path, primitive owner/resource/level, World Partition runtime-cell GUID/package/external layer, and actor/cell Data Layer membership.
- Capture each Data Layer's full name, object path, runtime/effective state, and whether membership came from the actor, runtime cell, or both.
- Carry the contributor ID as a third diagnostic sidecar beside each cached MPC uniform-buffer pointer. `BindingContributor` rows join contributor, binding lineage, and exact resource generation.
- Bound capture with `r.RHI.ResourceProvenance.MaxContributorDescriptors` and `r.RHI.ResourceProvenance.MaxDataLayersPerContributor`; log missing actor/runtime-cell and link counts at assertion time.
- Add `--binding` and `--contributor` decoder filters plus dedicated TSV columns. Feature 6 source commits are `097b76a7f17188850af8db25a9bb111a690cb49e` and coverage follow-up `572ac935c993d62136f2439a6f673755be2e1b05`.
- This is immutable contributor attribution at proxy creation. Feature 7 adds the later Data Layer state transitions needed to prove an unload/deactivation sequence.

### 2026-09-25 — Feature 5: release-time live cached-binding snapshots

- Add typed release causes for MPC asset begin/finish destroy, instance finish destroy, game-thread destroy, uniform-buffer recreation/invalid replacement, world instance replacement, post-GC invalid collection removal, and scene-map remove/replace.
- Retain up to 16 live cached-binding states per priority MPC generation without owning or dereferencing the underlying RHI resource.
- Track binding live-copy count, owner key, last operation/caller, invalidated state, and whether the binding was submitted.
- At every typed release cause, emit one `ReleaseBindingSnapshot` row per still-live binding and one `ReleaseBindingCoverage` row with active/omitted counts and whether tracking was enabled.
- Print the same retained binding state in the assertion log so the crash immediately distinguishes “resource released with live consumers” from a fully invalidated cache.
- Add typed decoder detail for release causes, binding snapshots, and coverage. Source commit: `465ed4b9fd5dd44e36af0a3ac0b0d736e78cd6aa`.
...
## 2026-09-25 — close the original ten-feature responsibility plan

The original ten requested capabilities have now been reconciled against the engine branch instead of relying on later internal feature numbering.

| # | Capability | Status | Concrete implementation |
|---|---|---|---|
| 1 | Targeted GC root-loss recorder | Complete | Existing pre/post-GC state is augmented with one healthy baseline and suspicious targeted `FReferenceChainSearch` snapshots for the configured MPC. |
| 2 | Asset load/unload provenance | Complete | Records MPC `PostLoad`, `BeginDestroy`, and `FinishDestroy`, plus `UAssetManager` bundle/load-complete/unload events with manager class, primary ID, handle addresses, bundle counts, object flags, linker state, and caller PC. |
| 3 | MPC generation lineage | Complete | Collection → world instance → render resource → exact uniform-buffer generation IDs are already linked. |
| 4 | Scene MPC map mutation audit | Complete | Existing insert/remove/old-new replacement rows remain unchanged. |
| 5 | Cached draw-command lifetime audit | Complete | Existing binding lineage, live-copy snapshots, invalidation, submission, and release rows remain unchanged. |
| 6 | Actor/component contributor tracking | Complete | Existing primitive, actor, component, material/proxy, cell, and Data Layer descriptors remain unchanged. |
| 7 | Bounded ref-operation census | Complete | Each promoted generation retains 32 AddRef/Release/final-release callsites, journals aggregate counts at `FinalRelease`, sets saturation bits, and emits omitted-operation coverage. Ordinary per-operation rows remain ring-only. |
| 8 | Cross-thread causal timeline | Complete | Existing game/render/RHI causal IDs, deferred command correlations, frame/cycle/thread data, cell transitions, and Data Layer transitions remain unchanged. |
| 9 | Targeted escalation after release | Complete | Existing selective final-release/physical-free/stale enqueue/stale execute stacks stay capped at 256 deeper command captures. |
| 10 | Automated responsibility report | Complete | Decoder `--responsibility-report --id <generation>` performs bounded multi-pass joins and emits a Markdown verdict, confidence, the five responsibility sections, exact evidence, and coverage gaps. |

### New recorder operations

Operations 71–79 were appended without changing the version-1 binary record layout:

- `MPCAssetPostLoad`, `MPCAssetBeginDestroy`, `MPCAssetFinishDestroy`;
- `MPCGCReferenceChain`;
- `ReferenceCensus`, `ReferenceCensusCoverage`;
- `MPCAssetManagerLoadRequest`, `MPCAssetManagerLoadComplete`, `MPCAssetManagerUnload`.

Reference census rows pack AddRef count in bits 0–11, Release count in bits 12–23, final-release count in bits 24–27, and saturation flags in bits 28–30. The row caller is the callsite PC; correlation stores its last observed cycle.

### Targeting and use

Default targeted root-chain settings:

`r.RHI.ResourceProvenance.MPCReferenceChains 1`  
`r.RHI.ResourceProvenance.MPCTarget MPC_GlobalEnvironment`  
`r.RHI.ResourceProvenance.MaxMPCReferenceChainCaptures 4`

Generate the joined report:

`python DecodeRHIResourceProvenance.py <journal.rhiprov> --id <generation-id> --responsibility-report --output responsibility.md`

Keep `r.RHI.ResourceProvenance.CommandUses 1` enabled for cached-binding and command responsibility. All new runtime code remains behind the existing Development/Debug provenance build gate; `GUniformBufferReleasehistory` is unchanged.

### Bounds and interpretation

- Reference-chain search is target-only, records one healthy baseline, then only suspicious states where the collection is neither rooted nor standalone.
- The reference census starts when a resource is promoted into the existing retained-priority table; earlier unassociated reference traffic remains available only in volatile rings.
- Text remains capped by the existing 256-TCHAR journal record and reports truncation through existing record flags.
- The responsibility report does not silently invent blame: a post-final-release use produces a high-confidence stale-binding/command verdict; incomplete joins are downgraded and listed under coverage gaps.
- Python syntax/help validation and a synthetic journal test covering Asset lifecycle → binding snapshot → final release → census → post-release submit/execute passed.
- This revision has not been compiled with the PS5 SDK/toolchain.

### Source revision

- Engine commit: `5ee8593e36126a9022dc5f2589d1a7c5a2638531`
- Changed source blobs:
  - `Engine/Source/Runtime/RHI/Public/RHIResourceProvenance.h`: `f86f40c258b6dc9daf9644bcaa9f1736929b2512`
  - `Engine/Source/Runtime/RHI/Private/RHIResourceProvenance.cpp`: `fec9125ea8af7a45a8915c0613dba1f0021b87d5`
  - `Engine/Source/Runtime/RHI/Public/RHIResources.h`: `6b37d78d0848edad480d40348d35ef391f708717`
  - `Engine/Source/Runtime/Engine/Public/ParameterCollection.h`: `318c4ccc86833bd24bc182002cab56a36296f664`
  - `Engine/Source/Runtime/Engine/Private/Materials/ParameterCollection.cpp`: `fd312932b954b47a958442ac4a03b78726a3acb2`
  - `Engine/Source/Runtime/Engine/Private/World.cpp`: `fb79b3df63b135a8985ba15ec6dbc764a7f7af7e`
...
### Exact Data Layer-to-cell causal fan-in
